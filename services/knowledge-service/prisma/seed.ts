/**
 * Knowledge Service — Seed Script
 *
 * Seeds all ~213 educational knowledge entries into app_knowledge.knowledge_entries.
 * Uses upsert (idempotent) — safe to re-run without duplicates.
 *
 * Usage:
 *   npm run db:seed              # Upsert all entries
 *   CLEAN_SEED=1 npm run db:seed # Delete all, then insert fresh
 */
...
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: process.env.KNOWLEDGE_DATABASE_URL || process.env.DATABASE_URL,
    },
  },
});
...
async function upsertEntry(entry: SeedEntry, index: number) {
  return prisma.knowledgeEntry.upsert({
    ...
    update: {
      ...
      title: entry.title,
      sanskrit: entry.sanskrit ?? null,
      summary: entry.summary,
      description: entry.description,
      howToRead: entry.howToRead ?? null,
...
// ╔══════════════════════════════════════════════════════════════════╗
// ║  SECTION 1: VEDIC — PANCHANGA (5 terms)                       ║
// ╚══════════════════════════════════════════════════════════════════╝

const PANCHANGA_ENTRIES: SeedEntry[] = [
  {
    termKey: "tithi",
    domain: "vedic",
    category: "panchanga",
...
Tithis are the single most important factor in Muhurta (electional astrology) — an auspicious tithi can uplift an otherwise mediocre muhurta, while an inauspicious tithi can undermine even a well-selected time.`,
    ...
    examples: [
      { title: "Shukla Chaturthi", content: "The 4th tithi of the bright half — sacred to Lord Ganesha. Ideal for beginning new ventures after offering Ganesha puja." },
      { title: "Krishna Ashtami", content: "The 8th tithi of the dark half — associated with Lord Krishna's birth (Janmashtami). A Rikta tithi, generally avoided for material beginnings." },
    ],
    relatedTerms: ["nakshatra", "karana", "yoga_panchanga", "vara", "paksha"],
    tags: ["panchanga", "lunar", "timing", "muhurta"],
  },
  {
    termKey: "nakshatra",
    domain: "vedic",
...
The Vimshottari Dasha system, the most widely used predictive timing tool, is entirely based on the birth nakshatra. The ruling planet of the birth nakshatra becomes the first Mahadasha lord, setting the entire life timeline.`,
    ...
    examples: [
      { title: "Ashwini Nakshatra", content: "Ruled by Ketu, deity Ashwini Kumaras (divine physicians). Symbol: Horse's head. Qualities: Speed, healing, pioneering. People born here are natural healers, fast-moving, and initiators." },
      { title: "Rohini Nakshatra", content: "Ruled by Moon, deity Brahma. Symbol: Ox cart. Krishna's birth nakshatra. Known for beauty, fertility, material abundance, and creative arts." },
    ],
    relatedTerms: ["tithi", "rashi", "vimshottari_dasha", "pada", "namakshar", "yoni", "gana"],
    tags: ["panchanga", "nakshatra", "lunar", "prediction", "dasha"],
  },
  {
...
Unlike tithis which change daily, yogas can shift at any point during the day. An auspicious yoga running during a critical moment (like signing a contract or starting a journey) is considered a positive reinforcement of the muhurta.`,
    ...
    examples: [
      { title: "Siddha Yoga", content: "One of the most auspicious yogas. Activities initiated under Siddha Yoga are said to 'automatically succeed' (siddhi = accomplishment). Ideal for starting businesses, signing agreements, or beginning studies." },
      { title: "Vyatipata Yoga", content: "Considered highly inauspicious — one of the two 'great evils' (Maha Doshas) in the Panchanga. Travel, marriage, and new ventures should be strictly avoided. However, it is considered favorable for propitiating ancestors." },
    ],
    relatedTerms: ["tithi", "nakshatra", "karana", "vara"],
    tags: ["panchanga", "timing", "muhurta", "auspicious"],
  },
  {
    termKey: "karana",
    domain: "vedic",
...
Karanas provide a finer temporal resolution than tithis, allowing astrologers to pinpoint exactly which half of a tithi is more favorable.`,
    ...
    examples: [
      { title: "Vishti (Bhadra) Karana", content: "Appears when Bhadra falls during specific tithis. Travel begun during Vishti Karana is said to cause obstacles. However, activities related to fire (cooking, metallurgy) and warfare are traditionally permitted." },
    ],
    relatedTerms: ["tithi", "nakshatra", "yoga_panchanga", "vara", "paksha"],
    tags: ["panchanga", "timing", "muhurta", "karana"],
  },
  {
    termKey: "vara",
    domain: "vedic",
...
Certain vara-tithi-nakshatra combinations create powerful auspicious or inauspicious yogas. For example, the Siddha Yoga formed by specific weekday-tithi pairs is one of the most sought-after muhurta indicators.`,
    ...
    examples: [
      { title: "Guru-Pushya Yoga", content: "When Thursday (Guru's day) coincides with Pushya Nakshatra, it creates one of the most auspicious combinations in the entire Panchanga — ideal for buying gold, starting education, or initiating any significant venture." },
    ],
    relatedTerms: ["tithi", "nakshatra", "yoga_panchanga", "karana", "hora_time"],
    tags: ["panchanga", "weekday", "timing", "muhurta"],
  },
];

// ╔══════════════════════════════════════════════════════════════════╗
// ║  SECTION 2: VEDIC — AVAKHADA CHAKRA (13 terms)                ║
// ╚══════════════════════════════════════════════════════════════════╝

const AVAKHADA_ENTRIES: SeedEntry[] = [
  {
    termKey: "varna",
    domain: "vedic",
    category: "avakhada",
...
In matchmaking (Ashta Koota), the groom's Varna should ideally be equal to or higher than the bride's. This scores 1 point out of 36 in the Koota system.`,
    ...
    relatedTerms: ["koota_varna", "vashya", "gana", "nadi"],
    tags: ["avakhada", "matchmaking", "temperament"],
  },
  {
    termKey: "vashya",
    domain: "vedic",
...
In matchmaking, Vashya scoring ranges from 0 to 2 points based on the mutual attraction/control relationship between the couple's Vashya categories.`,
    ...
    relatedTerms: ["koota_vashya", "varna", "rashi", "gana"],
    tags: ["avakhada", "matchmaking", "compatibility"],
  },
  {
    termKey: "yoni",
    domain: "vedic",
...
In matchmaking, Yoni scores 4 points — the highest among the first four Kootas — reflecting its significant influence on marital harmony, particularly in physical compatibility and instinctive behavior patterns.`,
    ...
    relatedTerms: ["koota_yoni", "nakshatra", "gana", "nadi"],
    tags: ["avakhada", "matchmaking", "compatibility", "nakshatra"],
  },
  {
    termKey: "gana",
    domain: "vedic",
...
In matchmaking, Deva-Deva scores 6, Manushya-Manushya scores 6, Deva-Manushya scores 5, Rakshasa-Rakshasa scores 6, but Deva-Rakshasa scores 0 — the worst combination for temperamental harmony.`,
    ...
    relatedTerms: ["koota_gana", "nakshatra", "varna", "yoni", "nadi"],
    tags: ["avakhada", "matchmaking", "temperament", "nakshatra"],
  },
  {
    termKey: "nadi",
    domain: "vedic",
...
Nadi carries the maximum weight of 8 points in Ashta Koota matching. Same Nadi scores 0 (Nadi Dosha), while different Nadis score 8 (full marks). This makes it the single most impactful factor in match scoring.`,
    ...
    relatedTerms: ["koota_naadi", "naadi_dosha", "nakshatra", "gana", "vashya"],
    tags: ["avakhada", "matchmaking", "health", "ayurveda", "nakshatra"],
  },
  {
    termKey: "tatva",
    domain: "vedic",
...
Complementary elements (Fire-Air, Earth-Water) tend to support each other, while conflicting elements (Fire-Water, Earth-Air) may create friction. Ether is generally compatible with all elements.`,
    ...
    relatedTerms: ["nakshatra", "gana", "nadi", "varna"],
    tags: ["avakhada", "element", "ayurveda", "nakshatra"],
  },
  {
    termKey: "rashi",
    domain: "vedic",
...
The Rashi is used extensively in transit analysis (Gochar), dasha interpretation, and as the reference point for Bhava (house) calculations in some systems.`,
    ...
    relatedTerms: ["nakshatra", "rashi_lord", "pada", "gochar"],
    tags: ["avakhada", "zodiac", "moon", "identity"],
  },
  {
    termKey: "rashi_lord",
    domain: "vedic",
...
Since the Moon represents the mind, the condition of its sign lord directly affects mental peace, decision-making ability, and overall life satisfaction.`,
    ...
    relatedTerms: ["rashi", "nakshatra", "sign_lord", "dignity"],
    tags: ["avakhada", "planet", "ruler", "moon"],
  },
  {
    termKey: "pada",
    domain: "vedic",
...
3. The specific shade of the nakshatra's qualities — Pada 1 (Dharma/purpose), Pada 2 (Artha/wealth), Pada 3 (Kama/desire), Pada 4 (Moksha/liberation)`,
    ...
    relatedTerms: ["nakshatra", "namakshar", "d9_navamsha", "rashi"],
    tags: ["avakhada", "nakshatra", "navamsha", "naming"],
  },
  {
    termKey: "namakshar",
    domain: "vedic",
...
Modern Indian families often use the Namakshar as the starting point, then find a meaningful name beginning with that syllable. Many families consult astrologers specifically for this purpose, especially for the formal/astrological name even if a different modern name is used socially.`,
    ...
    relatedTerms: ["nakshatra", "pada", "rashi"],
    tags: ["avakhada", "naming", "nakshatra", "ceremony"],
  },
  {
    termKey: "paya_rashi",
    domain: "vedic",
...
Paya analysis is often used alongside the more detailed Nakshatra-based Paya for a comprehensive assessment. When both Rashi Paya and Nakshatra Paya align (e.g., both Gold), the indication is strongly reinforced.`,
    ...
    relatedTerms: ["paya_nakshatra", "rashi", "nakshatra"],
    tags: ["avakhada", "wealth", "fortune", "metals"],
  },
  {
...
When both Rashi Paya and Nakshatra Paya agree, the prediction is strongly reinforced. When they conflict, the Nakshatra Paya is generally given more weight as it's more specific.`,
    ...
    relatedTerms: ["paya_rashi", "nakshatra", "rashi"],
    tags: ["avakhada", "wealth", "fortune", "nakshatra"],
  },
  {
    termKey: "paksha",
    domain: "vedic",
...
The Paksha is directly linked to the Tithi — Shukla Paksha contains tithis 1-15 (Pratipada to Purnima), and Krishna Paksha contains tithis 1-15 (Pratipada to Amavasya). The specific tithi within the paksha further refines the interpretation.`,
    ...
    relatedTerms: ["tithi", "nakshatra", "rashi"],
    tags: ["avakhada", "lunar", "moon", "panchanga"],
  },
];

// ╔══════════════════════════════════════════════════════════════════╗
// ║  SECTION 3: VEDIC — NAVAGRAHA (9 terms)                       ║
// ╚══════════════════════════════════════════════════════════════════╝

const NAVAGRAHA_ENTRIES: SeedEntry[] = [
  {
    termKey: "sun",
    domain: "vedic",
    category: "navagraha",
...
A strong Sun gives leadership ability, confidence, government favor, and good health. A weak or afflicted Sun can cause ego issues, problems with father/authority, heart ailments, and lack of direction.`,
    ...
    examples: [
      { title: "Sun in 10th House", content: "One of the most powerful placements — the Sun gets Dig Bala (directional strength) here. Indicates natural authority, government connections, and career prominence. The native often achieves a leadership position." },
      { title: "Sun-Saturn Conjunction", content: "Father may be distant or strict. The native struggles with authority figures early in life but develops tremendous discipline. Often seen in charts of self-made leaders who rise through persistent effort." },
    ],
    relatedTerms: ["moon", "mars", "jupiter", "dignity", "combustion"],
    tags: ["planet", "navagraha", "soul", "authority", "father"],
  },
  {
    termKey: "moon",
    domain: "vedic",
...
**Waxing vs Waning**: A waxing Moon (Shukla Paksha) is considered strong — bright, full, generous. A waning Moon (Krishna Paksha) is weaker — introverted, contemplative, anxious. This is why Paksha matters so much in chart assessment.`,
    ...
    examples: [
      { title: "Gaja Kesari Yoga", content: "When Jupiter is in a Kendra (1st, 4th, 7th, or 10th) from the Moon, it creates Gaja Kesari Yoga — one of the most celebrated yogas. Gives wisdom, wealth, and lasting fame." },
    ],
    relatedTerms: ["sun", "rashi", "nakshatra", "paksha", "gaja_kesari_yoga"],
    tags: ["planet", "navagraha", "mind", "emotions", "mother"],
  },
  {
    termKey: "mars",
    domain: "vedic",
...
**Manglik Dosha**: Mars in houses 1, 2, 4, 7, 8, or 12 from the Lagna or Moon creates Manglik Dosha — the most commonly discussed marriage affliction. It is said to cause conflict, aggression, or even separation in marriage if the partner is non-Manglik.`,
    ...
    relatedTerms: ["sun", "saturn", "manglik_dosha", "angarak_dosha"],
    tags: ["planet", "navagraha", "courage", "energy", "property"],
  },
  {
    termKey: "mercury",
    domain: "vedic",
...
**Special Quality**: Mercury is the most adaptable planet — it takes on the nature of planets it associates with. Mercury with Jupiter becomes wise and philosophical. Mercury with Mars becomes sharp and argumentative. Mercury with Saturn becomes methodical and research-oriented. This chameleon quality makes Mercury's conjunctions particularly important to analyze.`,
    ...
    relatedTerms: ["jupiter", "venus", "budha_aditya_yoga", "retrograde"],
    tags: ["planet", "navagraha", "intellect", "communication", "commerce"],
  },
  {
    termKey: "jupiter",
    domain: "vedic",
...
**Jupiter's Aspect**: Uniquely, Jupiter aspects the 5th, 7th, and 9th houses from its position (most planets only aspect the 7th). This triple aspect means Jupiter's benefic influence reaches far across the chart, blessing whichever houses it touches.`,
    ...
    relatedTerms: ["sun", "moon", "venus", "gaja_kesari_yoga", "guru_mangal_yoga", "guru_chandal_dosha"],
    tags: ["planet", "navagraha", "benefic", "wisdom", "wealth", "children"],
  },
  {
    termKey: "venus",
    domain: "vedic",
...
Venus is the second great benefic (after Jupiter). While Jupiter gives spiritual wisdom, Venus gives worldly wisdom — the ability to appreciate and create beauty, build harmonious relationships, and enjoy the material world without guilt.`,
    ...
    relatedTerms: ["mercury", "saturn", "mars", "manglik_dosha"],
    tags: ["planet", "navagraha", "benefic", "love", "beauty", "marriage"],
  },
  {
    termKey: "saturn",
    domain: "vedic",
...
**Sade Sati** — Saturn's most significant transit — occurs when Saturn transits the 12th, 1st, and 2nd houses from the Moon sign. This 7.5-year period is considered one of the most challenging phases of life, bringing restructuring, loss, and ultimately transformation.`,
    ...
    examples: [
      { title: "Sade Sati", content: "The 7.5-year Saturn transit over the Moon sign is life's great restructuring period. It strips away what isn't authentic and forces you to rebuild on solid foundations. Many people report that their greatest growth happened during Sade Sati." },
    ],
    relatedTerms: ["sun", "mars", "rahu", "sade_sati", "sani_dhaiya", "shrapit_dosha"],
    tags: ["planet", "navagraha", "malefic", "karma", "discipline", "longevity"],
  },
  {
    termKey: "rahu",
    domain: "vedic",
...
Rahu acts like an amplified version of the sign lord it occupies. Rahu in Venus's signs acts like an amplified Venus (extreme luxury, passion). Rahu in Mars's signs acts like amplified Mars (extreme aggression, risk-taking).`,
    ...
    relatedTerms: ["ketu", "saturn", "kala_sarpa_dosha", "guru_chandal_dosha", "angarak_dosha"],
    tags: ["planet", "navagraha", "shadow", "karma", "obsession", "technology"],
  },
  {
    termKey: "ketu",
    domain: "vedic",
...
Ketu gives opposite results to Rahu — where Rahu amplifies and obsesses, Ketu diminishes and detaches. The house Ketu occupies often represents an area where you feel "been there, done that" — naturally skilled but emotionally uninvested.`,
    ...
    relatedTerms: ["rahu", "mars", "kala_sarpa_dosha", "spiritual_yoga"],
    tags: ["planet", "navagraha", "shadow", "karma", "spirituality", "moksha"],
  },
];

// ══════════════════════════════════════════════════════════════════
// Placeholder sections for remaining content (to be filled in subsequent tasks)
// ══════════════════════════════════════════════════════════════════

// SECTION 4: VEDIC — ZODIAC SIGNS (12 terms) — Task S2.3
const RASHI_ENTRIES: SeedEntry[] = [
  {
    termKey: "aries",
    domain: "vedic",
    category: "rashi",
...
The Sun is exalted in Aries (at 10°), making this sign particularly conducive to authority, self-expression, and vitality. Saturn is debilitated here, indicating that patience and discipline don't come naturally to Aries energy.`,
    ...
    relatedTerms: ["mars", "sun", "saturn", "rashi"],
    tags: ["rashi", "zodiac", "fire", "cardinal"],
  },
  {
    termKey: "taurus",
    domain: "vedic",
...
Taurus natives are known for their reliability, love of comfort, and artistic sensibility. They build slowly but build to last. Their challenge is resistance to change — they can become possessive and inflexible.`,
    ...
    relatedTerms: ["venus", "moon", "rashi"],
    tags: ["rashi", "zodiac", "earth", "fixed"],
  },
  {
    termKey: "gemini",
    domain: "vedic",
...
Gemini natives excel at multitasking and social networking but may struggle with depth — they know a little about everything but may resist mastering one thing. Their gift is making complex information accessible and entertaining.`,
    ...
    relatedTerms: ["mercury", "rahu", "rashi"],
    tags: ["rashi", "zodiac", "air", "mutable"],
  },
  {
    termKey: "cancer",
    domain: "vedic",
...
Cancer natives are deeply caring, intuitive, and family-oriented. They have strong memories and hold onto both positive and negative experiences. Their challenge is mood fluctuation and tendency toward emotional manipulation when hurt.`,
    ...
    relatedTerms: ["moon", "jupiter", "mars", "rashi"],
    tags: ["rashi", "zodiac", "water", "cardinal"],
  },
  {
    termKey: "leo",
    domain: "vedic",
...
Leo natives are natural performers, leaders, and creators. They need an audience and thrive when appreciated. Their challenge is ego — the line between healthy self-confidence and arrogance can be thin.`,
    ...
    relatedTerms: ["sun", "rashi"],
    tags: ["rashi", "zodiac", "fire", "fixed"],
  },
  {
    termKey: "virgo",
    domain: "vedic",
...
Virgo natives are the problem-solvers of the zodiac — meticulous, health-conscious, and service-oriented. Their challenge is perfectionism and excessive criticism (of self and others).`,
    ...
    relatedTerms: ["mercury", "venus", "rashi"],
    tags: ["rashi", "zodiac", "earth", "mutable"],
  },
  {
    termKey: "libra",
    domain: "vedic",
...
Libra natives are natural diplomats, mediators, and aesthetes. They see all sides of every issue, which can be both their greatest strength (fairness) and greatest weakness (indecision).`,
    ...
    relatedTerms: ["venus", "saturn", "sun", "rashi"],
    tags: ["rashi", "zodiac", "air", "cardinal"],
  },
  {
    termKey: "scorpio",
    domain: "vedic",
...
Scorpio natives are intensely perceptive, emotionally powerful, and drawn to life's hidden dimensions. They are natural researchers, psychologists, and healers who understand transformation through crisis. Their challenge is trust — they can become suspicious, controlling, and vindictive when betrayed.`,
    ...
    relatedTerms: ["mars", "ketu", "moon", "rashi"],
    tags: ["rashi", "zodiac", "water", "fixed"],
  },
  {
    termKey: "sagittarius",
    domain: "vedic",
...
Sagittarius natives are born teachers, travelers, and seekers. They need freedom, philosophical engagement, and a sense of purpose. Their challenge is overconfidence and tactlessness — they can be blunt and preachy.`,
    ...
    relatedTerms: ["jupiter", "ketu", "rashi"],
    tags: ["rashi", "zodiac", "fire", "mutable"],
  },
  {
    termKey: "capricorn",
    domain: "vedic",
...
Capricorn natives are the builders and administrators of the zodiac. They understand hierarchy, respect tradition, and plan for the long term. Their challenge is rigidity and emotional coldness — they can sacrifice personal relationships for professional achievement.`,
    ...
    relatedTerms: ["saturn", "mars", "jupiter", "rashi"],
    tags: ["rashi", "zodiac", "earth", "cardinal"],
  },
  {
    termKey: "aquarius",
    domain: "vedic",
...
Aquarius natives are natural reformers, humanitarians, and group organizers. They think in terms of systems and communities rather than individuals. Their challenge is emotional detachment — they care about humanity abstractly but may struggle with individual intimacy.`,
    ...
    relatedTerms: ["saturn", "rahu", "rashi"],
    tags: ["rashi", "zodiac", "air", "fixed"],
  },
  {
    termKey: "pisces",
    domain: "vedic",
...
Pisces natives are deeply empathetic, spiritually inclined, and creatively gifted. They feel the suffering of others and often work in healing, artistic, or spiritual professions. Their challenge is maintaining boundaries — they absorb others' emotions and can become escapist.`,
    ...
    relatedTerms: ["jupiter", "venus", "mercury", "rashi"],
    tags: ["rashi", "zodiac", "water", "mutable"],
  },
];

// SECTION 5: VEDIC — NAKSHATRAS (27 terms) — Task S2.3
const NAKSHATRA_ENTRIES: SeedEntry[] = [
  {
    termKey: "ashwini", domain: "vedic", category: "nakshatra",
    title: "Ashwini (अश्विनी)", sanskrit: "अश्विनी", sortOrder: 1,
...
**Padas**: 1st (Aries navamsha — pure pioneer), 2nd (Taurus — practical healer), 3rd (Gemini — communicative healer), 4th (Cancer — empathetic healer)`,
    ...
    relatedTerms: ["ketu", "aries", "nakshatra", "vimshottari_dasha"],
    tags: ["nakshatra", "ketu", "healing", "speed"],
  },
  {
    termKey: "bharani", domain: "vedic", category: "nakshatra",
    title: "Bharani (भरणी)", sanskrit: "भरणी", sortOrder: 2,
...
Bharani is the nakshatra of extremes — extreme creativity, extreme discipline, extreme transformation. The combination of Venus (pleasure, creation) and Yama (death, restraint) creates people who deeply understand both sides of existence. They are often drawn to professions involving birth, death, or transformation — midwives, morticians, therapists, artists, judges.`,
    ...
    relatedTerms: ["venus", "aries", "nakshatra"],
    tags: ["nakshatra", "venus", "transformation", "creation"],
  },
  {
    termKey: "krittika", domain: "vedic", category: "nakshatra",
    title: "Krittika (कृत्तिका)", sanskrit: "कृत्तिका", sortOrder: 3,
...
The Aries portion (Pada 1) is more aggressive and pioneering, while the Taurus portion (Padas 2-4) is more nurturing and creative — like a cook who uses fire to transform raw ingredients into nourishment.`,
    ...
    relatedTerms: ["sun", "aries", "taurus", "nakshatra"],
    tags: ["nakshatra", "sun", "fire", "purification"],
  },
  {
    termKey: "rohini", domain: "vedic", category: "nakshatra",
    title: "Rohini (रोहिणी)", sanskrit: "रोहिणी", sortOrder: 4,
...
This is one of the best nakshatras for the Moon to occupy — giving emotional stability, appreciation for beauty, and natural magnetism.`,
    ...
    relatedTerms: ["moon", "taurus", "nakshatra"],
    tags: ["nakshatra", "moon", "beauty", "abundance"],
  },
  {
    termKey: "mrigashira", domain: "vedic", category: "nakshatra",
    title: "Mrigashira (मृगशिरा)", sanskrit: "मृगशिरा", sortOrder: 5,
...
Mrigashira natives are perpetual seekers — always searching for something (knowledge, love, meaning, beauty). The Taurus portion (Pada 1-2) searches in the material world, while the Gemini portion (Pada 3-4) searches in the intellectual realm.`,
    ...
    relatedTerms: ["mars", "taurus", "gemini", "nakshatra"],
    tags: ["nakshatra", "mars", "seeking", "curiosity"],
  },
  {
    termKey: "ardra", domain: "vedic", category: "nakshatra",
    title: "Ardra (आर्द्रा)", sanskrit: "आर्द्रा", sortOrder: 6,
...
Ardra natives often experience significant upheavals and transformations — storms that ultimately clear the air. They have exceptional analytical and research abilities (Rahu in Mercury's sign). Many brilliant scientists, technologists, and researchers have strong Ardra placements. The emotional intensity can manifest as both deep empathy and destructive rage.`,
    ...
    relatedTerms: ["rahu", "gemini", "nakshatra"],
    tags: ["nakshatra", "rahu", "transformation", "intellect"],
  },
  {
    termKey: "punarvasu", domain: "vedic", category: "nakshatra",
    title: "Punarvasu (पुनर्वसु)", sanskrit: "पुनर्वसु", sortOrder: 7,
...
Punarvasu natives have a remarkable ability to recover from setbacks. Jupiter's rulership gives them optimism and faith even in difficult times. The transition from Gemini (intellectual) to Cancer (emotional) in this nakshatra represents the journey from head to heart.`,
    ...
    relatedTerms: ["jupiter", "gemini", "cancer", "nakshatra"],
    tags: ["nakshatra", "jupiter", "renewal", "dharma"],
  },
  {
    termKey: "pushya", domain: "vedic", category: "nakshatra",
    title: "Pushya (पुष्य)", sanskrit: "पुष्य", sortOrder: 8,
...
Pushya is universally recommended in Muhurta for all auspicious activities. When Thursday (Guru's day) falls on Pushya Nakshatra, it creates "Guru-Pushya Yoga" — one of the most powerful positive combinations for buying gold, starting studies, and initiating ventures.`,
    ...
    relatedTerms: ["saturn", "cancer", "nakshatra", "muhurta"],
    tags: ["nakshatra", "saturn", "auspicious", "nurturing"],
  },
  {
    termKey: "ashlesha", domain: "vedic", category: "nakshatra",
    title: "Ashlesha (आश्लेषा)", sanskrit: "आश्लेषा", sortOrder: 9,
...
Ashlesha natives are hypnotically charismatic with piercing psychological insight. They can sense hidden motives and unspoken truths. The serpent energy gives them both healing power (like medical snake venom) and potential for manipulation. Many astrologers, psychologists, and occultists have prominent Ashlesha placements.`,
    ...
    relatedTerms: ["mercury", "cancer", "nakshatra", "rahu"],
    tags: ["nakshatra", "mercury", "serpent", "mystical"],
  },
  {
    termKey: "magha", domain: "vedic", category: "nakshatra",
    title: "Magha (मघा)", sanskrit: "मघा", sortOrder: 10,
...
Magha natives carry a natural air of authority and dignity. They feel a strong connection to their ancestors and family traditions. Ketu's rulership gives them past-life wisdom and a sense of having "been here before." They often hold positions of traditional authority — family heads, community leaders, or custodians of heritage.`,
    ...
    relatedTerms: ["ketu", "leo", "nakshatra", "pitra_dosha"],
    tags: ["nakshatra", "ketu", "ancestors", "authority"],
  },
  {
    termKey: "purva_phalguni", domain: "vedic", category: "nakshatra",
    title: "Purva Phalguni (पूर्वा फाल्गुनी)", sanskrit: "पूर्वा फाल्गुनी", sortOrder: 11,
...
Purva Phalguni natives are pleasure-loving, creative, and socially graceful. They have a talent for bringing joy to others and creating beautiful environments. This is considered one of the most romantic nakshatras — ideal for marriage and creative partnerships.`,
    ...
    relatedTerms: ["venus", "leo", "nakshatra"],
    tags: ["nakshatra", "venus", "love", "creativity"],
  },
  {
    termKey: "uttara_phalguni", domain: "vedic", category: "nakshatra",
    title: "Uttara Phalguni (उत्तरा फाल्गुनी)", sanskrit: "उत्तरा फाल्गुनी", sortOrder: 12,
...
The transition from Leo (fixed fire) to Virgo (mutable earth) makes Uttara Phalguni natives both confident and service-oriented — they lead by serving. This is considered an excellent nakshatra for marriage, as it emphasizes commitment and mutual support.`,
    ...
    relatedTerms: ["sun", "leo", "virgo", "nakshatra"],
    tags: ["nakshatra", "sun", "service", "contracts"],
  },
  {
    termKey: "hasta", domain: "vedic", category: "nakshatra",
    title: "Hasta (हस्त)", sanskrit: "हस्त", sortOrder: 13,
...
Hasta natives are skilled with their hands — surgeons, artists, craftspeople, magicians, pickpockets (in the negative expression). They have an innate ability to create, heal, and manipulate physical reality through manual skill. Palmistry is traditionally associated with this nakshatra.`,
    ...
    relatedTerms: ["moon", "virgo", "nakshatra"],
    tags: ["nakshatra", "moon", "craftsmanship", "healing"],
  },
  {
    termKey: "chitra", domain: "vedic", category: "nakshatra",
    title: "Chitra (चित्रा)", sanskrit: "चित्रा", sortOrder: 14,
...
Chitra natives are strikingly attractive and have an eye for design and beauty. Mars in Venus-ruled territory (Libra portion) creates dynamic aesthetic energy — they don't just appreciate beauty, they actively create it. Architects, fashion designers, jewelers, and graphic designers often have prominent Chitra placements.`,
    ...
    relatedTerms: ["mars", "virgo", "libra", "nakshatra", "lahiri_ayanamsa"],
    tags: ["nakshatra", "mars", "beauty", "architecture"],
  },
  {
    termKey: "swati", domain: "vedic", category: "nakshatra",
    title: "Swati (स्वाति)", sanskrit: "स्वाति", sortOrder: 15,
...
Swati natives are fiercely independent, excellent at business, and remarkably adaptable. Rahu in Venus's sign gives them ambitious material desires combined with diplomatic skill. They are natural traders, negotiators, and entrepreneurs — the self-made millionaires of the nakshatra system.`,
    ...
    relatedTerms: ["rahu", "libra", "nakshatra"],
    tags: ["nakshatra", "rahu", "independence", "trade"],
  },
  {
    termKey: "vishakha", domain: "vedic", category: "nakshatra",
    title: "Vishakha (विशाखा)", sanskrit: "विशाखा", sortOrder: 16,
...
Vishakha natives are goal-oriented to an extraordinary degree — once they set their sights on a target, nothing deters them. Jupiter's wisdom combined with Indra's ambition creates powerful leaders, politicians, and visionaries who shape society.`,
    ...
    relatedTerms: ["jupiter", "libra", "scorpio", "nakshatra"],
    tags: ["nakshatra", "jupiter", "determination", "ambition"],
  },
  {
    termKey: "anuradha", domain: "vedic", category: "nakshatra",
    title: "Anuradha (अनुराधा)", sanskrit: "अनुराधा", sortOrder: 17,
...
Anuradha natives are loyal friends, skilled organizers, and often find their greatest success in foreign lands or outside their community. The lotus symbolism is key — they bloom beautifully despite being rooted in challenging emotional (Scorpio) waters.`,
    ...
    relatedTerms: ["saturn", "scorpio", "nakshatra"],
    tags: ["nakshatra", "saturn", "friendship", "devotion"],
  },
  {
    termKey: "jyeshtha", domain: "vedic", category: "nakshatra",
    title: "Jyeshtha (ज्येष्ठा)", sanskrit: "ज्येष्ठा", sortOrder: 18,
...
The gandanta point (29°-30° Scorpio → 0° Sagittarius) at Jyeshtha's end is considered one of the most spiritually transformative — and challenging — birth positions.`,
    ...
    relatedTerms: ["mercury", "scorpio", "nakshatra"],
    tags: ["nakshatra", "mercury", "authority", "protection"],
  },
  {
    termKey: "moola", domain: "vedic", category: "nakshatra",
    title: "Moola (मूल)", sanskrit: "मूल", sortOrder: 19,
...
Moola births (especially in the first pada/gandanta) are traditionally viewed with concern, but the mature expression of Moola is that of the researcher, philosopher, and truth-seeker who won't stop until they reach the root cause.`,
    ...
    relatedTerms: ["ketu", "sagittarius", "nakshatra"],
    tags: ["nakshatra", "ketu", "roots", "investigation"],
  },
  {
    termKey: "purva_ashadha", domain: "vedic", category: "nakshatra",
    title: "Purva Ashadha (पूर्वा आषाढ़ा)", sanskrit: "पूर्वा आषाढ़ा", sortOrder: 20,
...
Purva Ashadha natives are philosophically passionate and verbally persuasive — the debaters, preachers, and advocates who argue their position with elegance and conviction. Venus in Sagittarius gives them the ability to present even harsh truths beautifully.`,
    ...
    relatedTerms: ["venus", "sagittarius", "nakshatra"],
    tags: ["nakshatra", "venus", "invincibility", "persuasion"],
  },
  {
    termKey: "uttara_ashadha", domain: "vedic", category: "nakshatra",
    title: "Uttara Ashadha (उत्तरा आषाढ़ा)", sanskrit: "उत्तरा आषाढ़ा", sortOrder: 21,
...
This nakshatra contains the Abhijit zone (end of Uttara Ashadha/beginning of Shravana) — considered so auspicious that it's sometimes counted as a 28th nakshatra.`,
    ...
    relatedTerms: ["sun", "sagittarius", "capricorn", "nakshatra", "abhijit_muhurta"],
    tags: ["nakshatra", "sun", "victory", "leadership"],
  },
  {
    termKey: "shravana", domain: "vedic", category: "nakshatra",
    title: "Shravana (श्रवण)", sanskrit: "श्रवण", sortOrder: 22,
...
Vishnu's three footprints symbolize the ability to span vast distances — physically (travel), intellectually (connecting ideas), and spiritually (linking heaven, earth, and the underworld).`,
    ...
    relatedTerms: ["moon", "capricorn", "nakshatra"],
    tags: ["nakshatra", "moon", "listening", "learning", "media"],
  },
  {
    termKey: "dhanishta", domain: "vedic", category: "nakshatra",
    title: "Dhanishta (धनिष्ठा)", sanskrit: "धनिष्ठा", sortOrder: 23,
...
Dhanishta natives often have natural musical talent — especially for percussion and rhythm. The drum symbol represents the heartbeat of the universe, and Dhanishta natives can feel and express cosmic rhythms. Wealth accumulation and fame are also strongly associated with this nakshatra.`,
    ...
    relatedTerms: ["mars", "capricorn", "aquarius", "nakshatra"],
    tags: ["nakshatra", "mars", "wealth", "music"],
  },
  {
    termKey: "shatabhisha", domain: "vedic", category: "nakshatra",
    title: "Shatabhisha (शतभिषा)", sanskrit: "शतभिषा", sortOrder: 24,
...
Shatabhisha natives are natural healers who often work through unconventional means — alternative medicine, energy healing, psychotherapy, or pharmaceutical research. The empty circle symbolizes both the void (shunya) and the wholeness that comes from perfect healing. Varuna's waters represent the healing power of the ocean and the cosmic laws that govern health.`,
    ...
    relatedTerms: ["rahu", "aquarius", "nakshatra"],
    tags: ["nakshatra", "rahu", "healing", "mysticism"],
  },
  {
    termKey: "purva_bhadrapada", domain: "vedic", category: "nakshatra",
    title: "Purva Bhadrapada (पूर्वा भाद्रपद)", sanskrit: "पूर्वा भाद्रपद", sortOrder: 25,
...
Purva Bhadrapada natives are philosophical warriors — they fight for ideas, principles, and spiritual truth with extreme intensity. The two-faced man symbol represents seeing both the worldly and the spiritual simultaneously. They can be austere ascetics or fiery revolutionaries (or both).`,
    ...
    relatedTerms: ["jupiter", "aquarius", "pisces", "nakshatra"],
    tags: ["nakshatra", "jupiter", "transformation", "spirituality"],
  },
  {
    termKey: "uttara_bhadrapada", domain: "vedic", category: "nakshatra",
    title: "Uttara Bhadrapada (उत्तरा भाद्रपद)", sanskrit: "उत्तरा भाद्रपद", sortOrder: 26,
...
Uttara Bhadrapada natives have oceanic depth — they are the wise elders, the silent meditators, the deep-sea divers of consciousness. Saturn's discipline channels Pisces's boundless spirituality into controlled, practical wisdom. They often appear quiet and unassuming but possess profound inner knowledge.`,
    ...
    relatedTerms: ["saturn", "pisces", "nakshatra"],
    tags: ["nakshatra", "saturn", "depth", "wisdom"],
  },
  {
    termKey: "revati", domain: "vedic", category: "nakshatra",
    title: "Revati (रेवती)", sanskrit: "रेवती", sortOrder: 27,
...
As the final nakshatra, Revati carries a sense of completion and cosmic compassion — the soul that has journeyed through all 26 previous stations now rests in universal love before the cycle begins again with Ashwini.`,
    ...
    relatedTerms: ["mercury", "pisces", "nakshatra", "ashwini"],
    tags: ["nakshatra", "mercury", "completion", "compassion"],
  },
];

// SECTION 6: VEDIC — DASHA SYSTEMS (13 terms) — Task S2.4
const DASHA_SYSTEM_ENTRIES: SeedEntry[] = [
  {
    termKey: "vimshottari_dasha", domain: "vedic", category: "dasha_system",
    title: "Vimshottari Dasha (विंशोत्तरी दशा)", sanskrit: "विंशोत्तरी दशा", sortOrder: 1,
...
**Why 120 years?** The Vedic tradition considers 120 years as the maximum natural human lifespan. The system is designed so that every person experiences all 9 planetary periods within a full lifetime.`,
    ...
    relatedTerms: ["mahadasha", "antardasha", "nakshatra", "moon"],
    tags: ["dasha", "timing", "prediction", "nakshatra"],
  },
  {
    termKey: "ashtottari_dasha", domain: "vedic", category: "dasha_system",
    title: "Ashtottari Dasha (अष्टोत्तरी दशा)", sanskrit: "अष्टोत्तरी दशा", sortOrder: 2,
...
Ashtottari is particularly popular in South India and is considered more suitable for certain chart configurations where Rahu plays a dominant role.`,
    ...
    relatedTerms: ["vimshottari_dasha", "rahu", "nakshatra"],
    tags: ["dasha", "timing", "prediction"],
  },
  {
    termKey: "tribhagi_dasha", domain: "vedic", category: "dasha_system",
    title: "Tribhagi Dasha (त्रिभागी दशा)", sanskrit: "त्रिभागी दशा", sortOrder: 3,
...
Tribhagi is used as a supplementary system alongside Vimshottari — it helps identify more precise timing windows within the broader Vimshottari periods. Some astrologers use it for shorter-term predictions.`,
    ...
    relatedTerms: ["vimshottari_dasha", "nakshatra"],
    tags: ["dasha", "timing", "prediction"],
  },
  {
    termKey: "shodashottari_dasha", domain: "vedic", category: "dasha_system",
    title: "Shodashottari Dasha (षोडशोत्तरी दशा)", sanskrit: "षोडशोत्तरी दशा", sortOrder: 4,
...
This system is recommended by Parasara for specific birth conditions and provides an alternative timeline that can clarify situations where Vimshottari alone gives ambiguous results.`,
    ...
    relatedTerms: ["vimshottari_dasha", "moon", "d2_hora"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "dwadashottari_dasha", domain: "vedic", category: "dasha_system",
    title: "Dwadashottari Dasha (द्वादशोत्तरी दशा)", sanskrit: "द्वादशोत्तरी दशा", sortOrder: 5,
...
Some texts specify that this system should be used when the birth nakshatra of the Lagna is ruled by Venus. It is particularly valued in charts where Jupiter and Saturn play dominant roles.`,
    ...
    relatedTerms: ["vimshottari_dasha", "shodashottari_dasha", "sun"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "panchottari_dasha", domain: "vedic", category: "dasha_system",
    title: "Panchottari Dasha (पंचोत्तरी दशा)", sanskrit: "पंचोत्तरी दशा", sortOrder: 6,
...
This system is relatively rare in practice but is preserved in classical texts as part of the complete dasha toolkit.`,
    ...
    relatedTerms: ["vimshottari_dasha", "cancer"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "chaturshitisama_dasha", domain: "vedic", category: "dasha_system",
    title: "Chaturshitisama Dasha (चतुर्शीतिसम दशा)", sanskrit: "चतुर्शीतिसम दशा", sortOrder: 7,
...
This system is applied when the 10th lord is in the 10th house — a very specific condition indicating a strong, well-defined career. The equal distribution reflects the balanced, structured life that such a chart typically produces.`,
    ...
    relatedTerms: ["vimshottari_dasha"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "satabdika_dasha", domain: "vedic", category: "dasha_system",
    title: "Satabdika Dasha (सतब्दिक दशा)", sanskrit: "सतब्दिक दशा", sortOrder: 8,
...
This system gives maximum weight to Saturn (30 years) and minimum to the luminaries (5 years each), reflecting the karmic emphasis in a chart where the body (Lagna) and mind (Moon) are in complete alignment.`,
    ...
    relatedTerms: ["vimshottari_dasha", "saturn", "moon"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "dwisaptati_dasha", domain: "vedic", category: "dasha_system",
    title: "Dwisaptati Sama Dasha (द्विसप्तति सम दशा)", sanskrit: "द्विसप्तति सम दशा", sortOrder: 9,
...
The equal distribution (like Chaturshitisama) reflects the balanced, partnership-oriented life that a strong 7th house signifies. Each planet gets exactly 9 years, removing the dominance of any single planetary period.`,
  ...
  {
    termKey: "shastihayani_dasha", domain: "vedic", category: "dasha_system",
    title: "Shastihayani Dasha (षष्टिहायणी दशा)", sanskrit: "षष्टिहायणी दशा", sortOrder: 10,
...
The 60-year cycle means all planetary periods are experienced more quickly, reflecting the intense, accelerated life trajectory of someone with the Sun in the Ascendant. Events come faster and with greater intensity.`,
    ...
    relatedTerms: ["vimshottari_dasha", "sun"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "shattrimshatsama_dasha", domain: "vedic", category: "dasha_system",
    title: "Shattrimshatsama Dasha (षट्त्रिंशत्सम दशा)", sanskrit: "षट्त्रिंशत्सम दशा", sortOrder: 11,
...
It is applied for births occurring during the daytime in Shukla Paksha (bright half of the lunar month). The extremely short periods (4-5 years each) mean life events shift rapidly — suitable for people who experience many changes and transitions.`,
    ...
    relatedTerms: ["vimshottari_dasha", "paksha"],
    tags: ["dasha", "timing", "conditional"],
  },
  {
    termKey: "chara_dasha", domain: "vedic", category: "dasha_system",
    title: "Chara Dasha (चर दशा)", sanskrit: "चर दशा", sortOrder: 12,
...
- Health crises (8th sign's dasha)`,
    ...
    relatedTerms: ["vimshottari_dasha", "rashi"],
    tags: ["dasha", "timing", "jaimini", "sign-based"],
  },
  {
    termKey: "tribhagi_40_dasha", domain: "vedic", category: "dasha_system",
    title: "Tribhagi 40 Dasha (त्रिभागी ४० दशा)", sanskrit: "त्रिभागी ४०", sortOrder: 13,
...
This system is used for focused short-range prediction, particularly effective for the first 40 years of life (education, marriage, career establishment) or when overlaid on any 40-year life segment.`,
    ...
    relatedTerms: ["vimshottari_dasha", "tribhagi_dasha"],
    tags: ["dasha", "timing", "prediction"],
  },
];

// SECTION 7: VEDIC — DASHA LEVELS (5 terms) — Task S2.4
const DASHA_LEVEL_ENTRIES: SeedEntry[] = [
  {
    termKey: "mahadasha", domain: "vedic", category: "dasha_level",
    title: "Mahadasha (महादशा)", sanskrit: "महादशा", sortOrder: 1,
...
The transition from one Mahadasha to another (called "Dasha Sandhi") is often a period of significant life change — relationships, career, health, and lifestyle may shift dramatically.`,
    ...
    relatedTerms: ["antardasha", "pratyantardasha", "vimshottari_dasha"],
    tags: ["dasha", "timing", "major_period"],
  },
  {
    termKey: "antardasha", domain: "vedic", category: "dasha_level",
    title: "Antardasha (अंतर्दशा)", sanskrit: "अंतर्दशा", sortOrder: 2,
...
- **6-8 or 2-12 relationship**: Difficult combination`,
    ...
    relatedTerms: ["mahadasha", "pratyantardasha", "vimshottari_dasha"],
    tags: ["dasha", "timing", "sub_period"],
  },
  {
    termKey: "pratyantardasha", domain: "vedic", category: "dasha_level",
    title: "Pratyantardasha (प्रत्यंतरदशा)", sanskrit: "प्रत्यंतरदशा", sortOrder: 3,
...
When the Pratyantardasha lord matches a transiting planet's signification, events crystallize with remarkable precision.`,
    ...
    relatedTerms: ["antardasha", "mahadasha", "sookshma_dasha"],
    tags: ["dasha", "timing", "precise"],
  },
  {
    termKey: "sookshma_dasha", domain: "vedic", category: "dasha_level",
    title: "Sookshma Dasha (सूक्ष्मदशा)", sanskrit: "सूक्ष्मदशा", sortOrder: 4,
...
At this level, the interplay of four planetary influences (Mahadasha → Antardasha → Pratyantardasha → Sookshma) creates a highly specific energetic signature that can only manifest in one specific way.`,
    ...
    relatedTerms: ["pratyantardasha", "prana_dasha"],
    tags: ["dasha", "timing", "micro"],
  },
  {
    termKey: "prana_dasha", domain: "vedic", category: "dasha_level",
    title: "Prana Dasha (प्राणदशा)", sanskrit: "प्राणदशा", sortOrder: 5,
...
The five-level chain (Mahadasha → Antardasha → Pratyantardasha → Sookshma → Prana) represents the complete hierarchy from decades to minutes — a fractal structure where the same planetary sequence operates at every temporal scale.`,
    ...
    relatedTerms: ["sookshma_dasha", "pratyantardasha"],
    tags: ["dasha", "timing", "finest"],
  },
];

// SECTION 8: VEDIC — DIVISIONAL CHARTS (16 terms) — Task S2.4
const VARGA_ENTRIES: SeedEntry[] = [
  {
    termKey: "d1_rashi", domain: "vedic", category: "divisional_chart",
    title: "D1 — Rashi Chart (राशि कुंडली)", sanskrit: "राशि कुंडली", sortOrder: 1,
...
**Shodashavarga weight**: 3.5 points (highest individual weight among the 16 vargas)`,
    ...
    relatedTerms: ["d9_navamsha", "rashi", "kundali"],
    tags: ["divisional_chart", "birth_chart", "fundamental"],
  },
  {
    termKey: "d2_hora", domain: "vedic", category: "divisional_chart",
    title: "D2 — Hora Chart (होरा)", sanskrit: "होरा", sortOrder: 2,
...
The word "Hora" itself derives from "Ahoratra" (day-night) with the first and last syllables removed — symbolizing the Sun-Moon duality that governs all material existence.`,
    ...
    relatedTerms: ["d1_rashi", "hora_time"],
    tags: ["divisional_chart", "wealth", "hora"],
  },
  {
    termKey: "d3_drekkana", domain: "vedic", category: "divisional_chart",
    title: "D3 — Drekkana Chart (द्रेक्काण)", sanskrit: "द्रेक्काण", sortOrder: 3,
...
- Self-expression and artistic abilities`,
    ...
    relatedTerms: ["d1_rashi", "mars"],
    tags: ["divisional_chart", "siblings", "courage"],
  },
  {
    termKey: "d4_chaturthamsha", domain: "vedic", category: "divisional_chart",
    title: "D4 — Chaturthamsha Chart (चतुर्थांश)", sanskrit: "चतुर्थांश", sortOrder: 4,
...
- Relationship with mother and maternal property`,
    ...
    relatedTerms: ["d1_rashi", "moon"],
    tags: ["divisional_chart", "property", "vehicles"],
  },
  {
    termKey: "d7_saptamsha", domain: "vedic", category: "divisional_chart",
    title: "D7 — Saptamsha Chart (सप्तांश)", sanskrit: "सप्तांश", sortOrder: 5,
...
- Creative intelligence and speculative ability`,
    ...
    relatedTerms: ["d1_rashi", "jupiter"],
    tags: ["divisional_chart", "children", "progeny"],
  },
  {
    termKey: "d9_navamsha", domain: "vedic", category: "divisional_chart",
    title: "D9 — Navamsha Chart (नवांश)", sanskrit: "नवांश", sortOrder: 6,
...
**Vargottama**: When a planet occupies the same sign in both D1 and D9, it is called Vargottama — one of the most powerful positive indicators, suggesting that the planet's external expression and inner nature are aligned.`,
    ...
    relatedTerms: ["d1_rashi", "pada", "nakshatra"],
    tags: ["divisional_chart", "marriage", "dharma", "navamsha"],
  },
  {
    termKey: "d10_dashamsha", domain: "vedic", category: "divisional_chart",
    title: "D10 — Dashamsha Chart (दशांश)", sanskrit: "दशांश", sortOrder: 7,
...
D10 is essential for career guidance — while D1 shows general career potential, D10 reveals the specific professional domain, the level of success achievable, and the timing of career peaks and valleys.`,
    ...
    relatedTerms: ["d1_rashi", "sun", "saturn"],
    tags: ["divisional_chart", "career", "profession"],
  },
  {
    termKey: "d12_dwadashamsha", domain: "vedic", category: "divisional_chart",
    title: "D12 — Dwadashamsha Chart (द्वादशांश)", sanskrit: "द्वादशांश", sortOrder: 8,
...
- Ancestral patterns and inherited traits`,
    ...
    relatedTerms: ["d1_rashi", "sun", "moon"],
    tags: ["divisional_chart", "parents", "lineage"],
  },
  {
    termKey: "d16_shodashamsha", domain: "vedic", category: "divisional_chart",
    title: "D16 — Shodashamsha Chart (षोडशांश)", sanskrit: "षोडशांश", sortOrder: 9,
...
In modern context, D16 extends beyond traditional vehicles (horses, elephants, chariots) to include cars, aircraft, ships, and all modes of transportation. It also governs the happiness derived from material possessions and physical comfort.`,
    ...
    relatedTerms: ["d1_rashi", "d4_chaturthamsha"],
    tags: ["divisional_chart", "vehicles", "comfort"],
  },
  {
    termKey: "d20_vimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D20 — Vimshamsha Chart (विंशांश)", sanskrit: "विंशांश", sortOrder: 10,
...
- Religious inclinations and preferred deities`,
    ...
    relatedTerms: ["d1_rashi", "jupiter", "ketu"],
    tags: ["divisional_chart", "spirituality", "worship"],
  },
  {
    termKey: "d24_chaturvimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D24 — Chaturvimshamsha Chart (चतुर्विंशांश)", sanskrit: "चतुर्विंशांश", sortOrder: 11,
...
- Teaching ability and knowledge transmission`,
    ...
    relatedTerms: ["d1_rashi", "mercury", "jupiter"],
    tags: ["divisional_chart", "education", "learning"],
  },
  {
    termKey: "d27_saptavimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D27 — Saptavimshamsha Chart (सप्तविंशांश)", sanskrit: "सप्तविंशांश", sortOrder: 12,
...
This chart is unique because its 27-fold division mirrors the nakshatra system, creating a direct link between divisional chart analysis and nakshatra-based assessment. It reveals the core constitutional strength of the native.`,
    ...
    relatedTerms: ["d1_rashi", "nakshatra"],
    tags: ["divisional_chart", "strength", "constitution"],
  },
  {
    termKey: "d30_trimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D30 — Trimshamsha Chart (त्रिंशांश)", sanskrit: "त्रिंशांश", sortOrder: 13,
...
D30 is sometimes called the "evil" chart — but more accurately, it maps the native's vulnerability landscape, allowing preventive measures.`,
    ...
    relatedTerms: ["d1_rashi", "mars", "saturn"],
    tags: ["divisional_chart", "health", "misfortune"],
  },
  {
    termKey: "d40_chatvarimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D40 — Chatvarimshamsha Chart (चत्वारिंशांश)", sanskrit: "चत्वारिंशांश", sortOrder: 14,
...
- The "lucky breaks" factor in a person's life`,
    ...
    relatedTerms: ["d1_rashi", "d45_panchavimshamsha"],
    tags: ["divisional_chart", "karma", "merit"],
  },
  {
    termKey: "d45_panchavimshamsha", domain: "vedic", category: "divisional_chart",
    title: "D45 — Akshavedamsha Chart (अक्षवेदांश)", sanskrit: "अक्षवेदांश", sortOrder: 15,
...
D45 complements D40 — where D40 shows accumulated merit, D45 shows current-life character and conduct. Together, they paint a complete picture of the native's karmic trajectory: past merit (D40) plus current conduct (D45) = future destiny.`,
    ...
    relatedTerms: ["d1_rashi", "d40_chatvarimshamsha"],
    tags: ["divisional_chart", "character", "ethics"],
  },
  {
    termKey: "d60_shashtiamsha", domain: "vedic", category: "divisional_chart",
    title: "D60 — Shashtiamsha Chart (षष्ठिांश)", sanskrit: "षष्ठिांश", sortOrder: 16,
...
**Critical precision requirement**: D60 requires extremely accurate birth time — a 30-second error in birth time shifts the D60 positions by one division. This makes D60 both the most revealing and the most demanding chart.`,
    ...
    relatedTerms: ["d1_rashi", "d9_navamsha"],
    tags: ["divisional_chart", "past_life", "karma", "finest"],
  },
];

// SECTION 9: VEDIC — YOGAS (11 terms) — Task S2.5
const YOGA_ENTRIES: SeedEntry[] = [
  {
    termKey: "gaja_kesari_yoga", domain: "vedic", category: "yoga",
    title: "Gaja Kesari Yoga (गजकेसरी योग)", sanskrit: "गजकेसरी योग", sortOrder: 1,
...
**Important caveat:** This yoga is quite common (Jupiter is in a Kendra from Moon for roughly 33% of people). The strength of results depends on the dignity and overall condition of both Jupiter and Moon.`,
    ...
    relatedTerms: ["jupiter", "moon", "raja_yoga"],
    tags: ["yoga", "benefic", "jupiter", "moon", "fame"],
  },
  {
    termKey: "guru_mangal_yoga", domain: "vedic", category: "yoga",
    title: "Guru Mangal Yoga (गुरु मंगल योग)", sanskrit: "गुरु मंगल योग", sortOrder: 2,
...
This yoga is particularly powerful when formed in Kendra houses (1, 4, 7, 10) and when both planets are well-dignified. Mars provides the energy to act, Jupiter provides the wisdom to act rightly.`,
    ...
    relatedTerms: ["jupiter", "mars", "raja_yoga"],
    tags: ["yoga", "benefic", "jupiter", "mars"],
  },
  {
    termKey: "budha_aditya_yoga", domain: "vedic", category: "yoga",
    title: "Budha Aditya Yoga (बुधादित्य योग)", sanskrit: "बुधादित्य योग", sortOrder: 3,
...
The true power of Budha Aditya lies in the house placement and sign — in the 10th house, it creates powerful administrative ability; in the 5th, it gives exceptional educational achievements.`,
    ...
    relatedTerms: ["sun", "mercury", "combustion"],
    tags: ["yoga", "benefic", "sun", "mercury", "intellect"],
  },
  {
    termKey: "chandra_mangal_yoga", domain: "vedic", category: "yoga",
    title: "Chandra Mangal Yoga (चन्द्र मंगल योग)", sanskrit: "चन्द्र मंगल योग", sortOrder: 4,
...
**Caveat:** Moon-Mars conjunction can also create emotional volatility if afflicted — anger, impulsiveness, and emotional reactions that damage relationships. The sign and house placement determine whether this yoga expresses constructively or destructively.`,
    ...
    relatedTerms: ["moon", "mars"],
    tags: ["yoga", "benefic", "moon", "mars", "wealth"],
  },
  {
    termKey: "raja_yoga", domain: "vedic", category: "yoga",
    title: "Raja Yoga (राजयोग)", sanskrit: "राजयोग", sortOrder: 5,
...
Not all Raja Yogas are equal — the strength depends on the planets involved, their dignity, and the houses they connect. A Raja Yoga formed by naturally benefic planets (Jupiter, Venus) is more readily activated than one formed by malefics.`,
    ...
    relatedTerms: ["gaja_kesari_yoga", "dhana_yoga", "pancha_mahapurusha_yoga"],
    tags: ["yoga", "benefic", "power", "authority", "kendra", "trikona"],
  },
  {
    termKey: "pancha_mahapurusha_yoga", domain: "vedic", category: "yoga",
    title: "Pancha Mahapurusha Yoga (पंच महापुरुष योग)", sanskrit: "पंच महापुरुष योग", sortOrder: 6,
...
**Important**: These yogas are moderately common individually (each occurs in roughly 15-20% of charts) but their strength varies enormously based on the planet's house placement, aspects, and conjunction with other planets.`,
    ...
    relatedTerms: ["raja_yoga", "dignity"],
    tags: ["yoga", "benefic", "personality", "archetype"],
  },
  {
    termKey: "dhana_yoga", domain: "vedic", category: "yoga",
    title: "Dhana Yoga (धनयोग)", sanskrit: "धनयोग", sortOrder: 7,
...
- Multiple Dhana Yogas: Extreme wealth potential`,
    ...
    relatedTerms: ["raja_yoga", "d2_hora"],
    tags: ["yoga", "benefic", "wealth", "finance"],
  },
  {
    termKey: "shubha_yoga", domain: "vedic", category: "yoga",
    title: "Shubha Yoga (शुभयोग)", sanskrit: "शुभयोग", sortOrder: 8,
...
A chart with multiple Shubha Yogas — benefics well-placed across Kendras and Trikonas — produces a generally fortunate life with strong social support, good health, and ethical success.`,
    ...
    relatedTerms: ["raja_yoga", "dhana_yoga", "jupiter", "venus"],
    tags: ["yoga", "benefic", "general", "fortune"],
  },
  {
    termKey: "kalpadruma_yoga", domain: "vedic", category: "yoga",
    title: "Kalpadruma Yoga (कल्पद्रुम योग)", sanskrit: "कल्पद्रुम योग", sortOrder: 9,
...
**Results:** The native's desires are fulfilled with minimal effort — opportunities come naturally, the right people appear at the right time, and intentions manifest into reality. This yoga gives the ability to attract what is needed.`,
    ...
    relatedTerms: ["raja_yoga"],
    tags: ["yoga", "benefic", "wish_fulfillment", "lagna"],
  },
  {
    termKey: "spiritual_yoga", domain: "vedic", category: "yoga",
    title: "Spiritual Yoga (आध्यात्मिक योग)", sanskrit: "आध्यात्मिक योग", sortOrder: 10,
...
**Signs of activation:** Attraction to meditation, disinterest in material accumulation, seeking solitude, deep philosophical questioning, experiences of transcendence, and gravitating toward spiritual teachers.`,
    ...
    relatedTerms: ["ketu", "jupiter", "saturn", "d20_vimshamsha"],
    tags: ["yoga", "spiritual", "moksha", "liberation"],
  },
  {
    termKey: "viparitha_raja_yoga", domain: "vedic", category: "yoga",
    title: "Viparitha Raja Yoga (विपरीत राजयोग)", sanskrit: "विपरीत राजयोग", sortOrder: 11,
...
**The paradox:** These natives often rise to power through adversity — the very difficulties that would crush others become their stepping stones. Many self-made millionaires and crisis leaders have Viparitha Raja Yoga.`,
    ...
    relatedTerms: ["raja_yoga"],
    tags: ["yoga", "benefic", "adversity", "transformation"],
  },
];

// SECTION 10: VEDIC — DOSHAS (7 terms) — Task S2.5
const DOSHA_ENTRIES: SeedEntry[] = [
  {
    termKey: "kala_sarpa_dosha", domain: "vedic", category: "dosha",
    title: "Kala Sarpa Dosha (कालसर्प दोष)", sanskrit: "कालसर्प दोष", sortOrder: 1,
...
- **Not always negative:** Some of the most powerful charts in history had Kala Sarpa — it concentrates energy rather than dispersing it`,
    ...
    relatedTerms: ["rahu", "ketu", "shrapit_dosha"],
    tags: ["dosha", "rahu", "ketu", "karmic"],
  },
  {
    termKey: "shrapit_dosha", domain: "vedic", category: "dosha",
    title: "Shrapit Dosha (श्रापित दोष)", sanskrit: "श्रापित दोष", sortOrder: 2,
...
**Remedies:** Shrapit Dosha remedies emphasize karmic correction — charitable actions, particularly feeding the needy, donating to causes related to the affected house, and regular recitation of Saturn and Rahu mantras.`,
    ...
    relatedTerms: ["saturn", "rahu", "kala_sarpa_dosha", "pitra_dosha"],
    tags: ["dosha", "saturn", "rahu", "karmic", "curse"],
  },
  {
    termKey: "pitra_dosha", domain: "vedic", category: "dosha",
    title: "Pitra Dosha (पितृ दोष)", sanskrit: "पितृ दोष", sortOrder: 3,
...
**Remedies:** Performing Shraddha (ancestral rites), Tarpana (water offerings), feeding Brahmins on Amavasya (new moon), and Pitra Paksha rituals.`,
    ...
    relatedTerms: ["sun", "rahu", "magha", "shrapit_dosha"],
    tags: ["dosha", "ancestors", "karmic", "remedies"],
  },
  {
    termKey: "guru_chandal_dosha", domain: "vedic", category: "dosha",
    title: "Guru Chandal Dosha (गुरु चण्डाल दोष)", sanskrit: "गुरु चण्डाल दोष", sortOrder: 4,
...
**Important:** Guru Chandal is NOT inherently destructive — in certain houses and signs, it can produce unconventional wisdom, innovative teaching, and success in foreign lands. Context determines whether it's a "polluted guru" or a "revolutionary thinker."`,
    ...
    relatedTerms: ["jupiter", "rahu", "shrapit_dosha"],
    tags: ["dosha", "jupiter", "rahu", "wisdom", "deception"],
  },
  {
    termKey: "angarak_dosha", domain: "vedic", category: "dosha",
    title: "Angarak Dosha (अंगारक दोष)", sanskrit: "अंगारक दोष", sortOrder: 5,
...
**Positive expression:** When well-channeled, Angarak Dosha produces exceptional courage, military/police aptitude, surgical precision, and success in competitive sports. Many successful athletes and military leaders have this combination.`,
    ...
    relatedTerms: ["mars", "rahu"],
    tags: ["dosha", "mars", "rahu", "anger", "accidents"],
  },
  {
    termKey: "sade_sati", domain: "vedic", category: "dosha",
    title: "Sade Sati (साढ़े साती)", sanskrit: "साढ़े साती", sortOrder: 6,
...
**Modern understanding:** Sade Sati is NOT a curse — it's Saturn's restructuring program. Whatever in your life is built on shaky foundations gets demolished so it can be rebuilt properly. Relationships that are genuine survive and strengthen; those that aren't fall away.`,
    ...
    relatedTerms: ["saturn", "moon", "sani_dhaiya"],
    tags: ["dosha", "saturn", "transit", "restructuring"],
  },
  {
    termKey: "sani_dhaiya", domain: "vedic", category: "dosha",
    title: "Sani Dhaiya (ढैया)", sanskrit: "ढैया", sortOrder: 7,
...
**Comparative severity:** Sade Sati > Ashtama Shani > Kantak Shani. However, the impact is modulated by Saturn's natal strength, the Moon's condition, and any protective aspects from Jupiter.`,
    ...
    relatedTerms: ["saturn", "moon", "sade_sati"],
    tags: ["dosha", "saturn", "transit", "challenges"],
  },
];

// SECTION 11: VEDIC — SHADBALA (6+3 terms) — Task S2.5
const SHADBALA_ENTRIES: SeedEntry[] = [
  {
    termKey: "sthana_bala", domain: "vedic", category: "shadbala",
    title: "Sthana Bala (स्थान बल)", sanskrit: "स्थान बल", sortOrder: 1,
...
Sthana Bala answers: "How well-positioned is this planet in the cosmic geography?"`,
    ...
    relatedTerms: ["dig_bala", "kala_bala", "virupa", "rupa"],
    tags: ["shadbala", "strength", "position"],
  },
  {
    termKey: "dig_bala", domain: "vedic", category: "shadbala",
    title: "Dig Bala (दिग बल)", sanskrit: "दिग बल", sortOrder: 2,
...
Dig Bala is calculated as proportional to the planet's angular distance from its point of maximum strength. At the opposite point (e.g., Sun in the 4th), Dig Bala is zero.`,
    ...
    relatedTerms: ["sthana_bala", "kala_bala"],
    tags: ["shadbala", "strength", "direction", "house"],
  },
  {
    termKey: "kala_bala", domain: "vedic", category: "shadbala",
    title: "Kala Bala (काल बल)", sanskrit: "काल बल", sortOrder: 3,
...
Kala Bala answers: "Was this planet temporally empowered at the moment of birth?"`,
    ...
    relatedTerms: ["sthana_bala", "dig_bala", "paksha"],
    tags: ["shadbala", "strength", "time", "temporal"],
  },
  {
    termKey: "cheshta_bala", domain: "vedic", category: "shadbala",
    title: "Cheshta Bala (चेष्ट बल)", sanskrit: "चेष्ट बल", sortOrder: 4,
...
**Why retrograde = strong**: A retrograde planet is astronomically closer to Earth, appearing brighter and larger. In Vedic astrology, this proximity translates to more intense, internalized, and powerful expression — though the expression may be unconventional or delayed.`,
    ...
    relatedTerms: ["retrograde", "sthana_bala"],
    tags: ["shadbala", "strength", "motion", "retrograde"],
  },
  {
    termKey: "naisargika_bala", domain: "vedic", category: "shadbala",
    title: "Naisargika Bala (नैसर्गिक बल)", sanskrit: "नैसर्गिक बल", sortOrder: 5,
...
**Purpose**: Naisargika Bala ensures that when two planets are otherwise equally strong, the naturally more luminous planet prevails. It's the "tiebreaker" in planetary strength comparisons.`,
    ...
    relatedTerms: ["sthana_bala", "dig_bala"],
    tags: ["shadbala", "strength", "natural", "inherent"],
  },
  {
    termKey: "drik_bala", domain: "vedic", category: "shadbala",
    title: "Drik Bala (दृक बल)", sanskrit: "दृक बल", sortOrder: 6,
...
Drik Bala answers: "Is this planet being helped or harmed by the cosmic environment around it?"`,
    ...
    relatedTerms: ["sthana_bala", "jupiter"],
    tags: ["shadbala", "strength", "aspects", "influence"],
  },
  {
    termKey: "virupa", domain: "vedic", category: "shadbala",
    title: "Virupa (विरुप)", sanskrit: "विरुप", sortOrder: 7,
...
A planet exceeding its minimum required Shadbala is considered "strong" — it can deliver its significations effectively.`,
    ...
    relatedTerms: ["rupa", "sthana_bala"],
    tags: ["shadbala", "measurement", "unit"],
  },
  {
    termKey: "rupa", domain: "vedic", category: "shadbala",
    title: "Rupa (रुप)", sanskrit: "रुप", sortOrder: 8,
...
**Practical application:** When Shadbala software shows a planet at 8.5 Rupas against a minimum requirement of 5.0, that planet is at 170% strength — well above the threshold and capable of strong expression.`,
    ...
    relatedTerms: ["virupa", "sthana_bala"],
    tags: ["shadbala", "measurement", "unit"],
  },
  {
    termKey: "ishta_kashta", domain: "vedic", category: "shadbala",
    title: "Ishta-Kashta Phala (इष्ट-कष्ट फल)", sanskrit: "इष्ट-कष्ट", sortOrder: 9,
...
A planet can be numerically strong (high total Rupas) but have more Kashta than Ishta — meaning its strength produces challenges rather than benefits. Conversely, a moderately strong planet with high Ishta and low Kashta is functionally more beneficial.`,
    ...
    relatedTerms: ["virupa", "rupa", "cheshta_bala"],
    tags: ["shadbala", "measurement", "beneficial", "harmful"],
  },
];

// SECTION 12: VEDIC — ASHTAKAVARGA (5 terms) — Task S2.5
const ASHTAKAVARGA_ENTRIES: SeedEntry[] = [
  {
    termKey: "sarvashtakavarga", domain: "vedic", category: "ashtakavarga",
    title: "Sarvashtakavarga (सर्वाष्टकवर्ग)", sanskrit: "सर्वाष्टकवर्ग", sortOrder: 1,
...
**Transit application**: SAV is most powerful for predicting transit results. When Saturn, Jupiter, or any planet transits a high-SAV sign in your chart, positive results follow. Low-SAV transits bring challenges.`,
    ...
    relatedTerms: ["bhinnashtakavarga", "shodasha_ashtakavarga"],
    tags: ["ashtakavarga", "strength", "transit"],
  },
  {
    termKey: "bhinnashtakavarga", domain: "vedic", category: "ashtakavarga",
    title: "Bhinnashtakavarga (भिन्नाष्टकवर्ग)", sanskrit: "भिन्नाष्टकवर्ग", sortOrder: 2,
...
BAV is the most powerful tool for transit prediction. When Jupiter transits a sign where its BAV score is 6+, expect excellent results in Jupiter's significations.`,
    ...
    relatedTerms: ["sarvashtakavarga", "shodasha_ashtakavarga"],
    tags: ["ashtakavarga", "strength", "planet"],
  },
  {
    termKey: "shodasha_ashtakavarga", domain: "vedic", category: "ashtakavarga",
    title: "Shodasha Ashtakavarga (षोडशवर्ग अष्टकवर्ग)", sanskrit: "षोडशवर्ग अष्टकवर्ग", sortOrder: 3,
...
**Application:** Used by advanced astrologers for confirming Dasha predictions, refining transit timing, and resolving ambiguous chart readings where standard Ashtakavarga gives borderline scores.`,
    ...
    relatedTerms: ["sarvashtakavarga", "bhinnashtakavarga"],
    tags: ["ashtakavarga", "strength", "advanced"],
  },
  {
    termKey: "tatkalik_maitri", domain: "vedic", category: "ashtakavarga",
    title: "Tatkalik Maitri Chakra (ताकालिक मैत्री चक्र)", sanskrit: "ताकालिक मैत्री", sortOrder: 4,
...
This combined assessment determines how planets actually interact in a specific chart, beyond generic natural friendship rules.`,
    ...
    relatedTerms: ["sarvashtakavarga"],
    tags: ["ashtakavarga", "friendship", "relationship"],
  },
  {
    termKey: "karaka_ashtakavarga", domain: "vedic", category: "ashtakavarga",
    title: "Karaka Ashtakavarga (कारक अष्टकवर्ग)", sanskrit: "कारक अष्टकवर्ग", sortOrder: 5,
...
When the Karaka planet has high BAV score (5+) in its signification house, the life area thrives. Low BAV (below 3) indicates challenges in that domain.`,
    ...
    relatedTerms: ["bhinnashtakavarga", "sarvashtakavarga"],
    tags: ["ashtakavarga", "karaka", "significator"],
  },
];

// SECTION 13: VEDIC — DIGNITY (4 terms) — Task S2.5
const DIGNITY_ENTRIES: SeedEntry[] = [
  {
    termKey: "ucha", domain: "vedic", category: "dignity",
    title: "Ucha — Exaltation (उच्च)", sanskrit: "उच्च", sortOrder: 1,
...
An exalted planet is like a king in his own palace — naturally commanding, confident, and powerful. It produces excellent results in its Dasha and strongly supports whatever house it occupies.`,
    ...
    relatedTerms: ["neecha", "swakshetra", "vimsopaka", "dignity"],
    tags: ["dignity", "exaltation", "strength"],
  },
  {
    termKey: "neecha", domain: "vedic", category: "dignity",
    title: "Neecha — Debilitation (नीच)", sanskrit: "नीच", sortOrder: 2,
...
Neecha Bhanga Raja Yoga — when debilitation is cancelled, it can paradoxically produce exceptional results, often surpassing normal exaltation.`,
    ...
    relatedTerms: ["ucha", "swakshetra", "vimsopaka", "dignity"],
    tags: ["dignity", "debilitation", "weakness"],
  },
  {
    termKey: "swakshetra", domain: "vedic", category: "dignity",
    title: "Swakshetra — Own Sign (स्व क्षेत्र)", sanskrit: "स्व क्षेत्र", sortOrder: 3,
...
A Swakshetra planet is self-sufficient — it doesn't need external support (aspects from benefics) to deliver results. It generates its own strength from being in its natural domain.`,
    ...
    relatedTerms: ["ucha", "neecha", "vimsopaka", "dignity"],
    tags: ["dignity", "own_sign", "strength"],
  },
  {
    termKey: "vimsopaka", domain: "vedic", category: "dignity",
    title: "Vimsopaka Bala (विंशोपक बल)", sanskrit: "विंशोपक बल", sortOrder: 4,
...
The Shodashavarga calculation gives the most comprehensive assessment, as it considers dignity across all life areas (career, marriage, children, spirituality, etc.).`,
    ...
    relatedTerms: ["ucha", "neecha", "swakshetra", "d1_rashi", "d9_navamsha"],
    tags: ["dignity", "composite", "score", "vimsopaka"],
  },
];

// SECTION 14: VEDIC — PLANETARY ATTRIBUTES (8 terms) — Task S2.5
const ATTRIBUTE_ENTRIES: SeedEntry[] = [
  {
    termKey: "sign_lord", domain: "vedic", category: "planetary_attribute",
    title: "Sign Lord (राशि अधिपति)", sanskrit: "राशि अधिपति", sortOrder: 1,
...
**Key principle:** Always check the sign lord's position, dignity, and strength alongside any planet's placement. A planet in a friendly sign with a strong sign lord produces significantly better results than the same planet in the same sign with a weak sign lord.`,
    ...
    relatedTerms: ["rashi_lord", "nakshatra_lord", "dignity"],
    tags: ["attribute", "ruler", "sign"],
  },
  {
    termKey: "nakshatra_lord", domain: "vedic", category: "planetary_attribute",
    title: "Nakshatra Lord (नक्षत्र स्वामी)", sanskrit: "नक्षत्र स्वामी", sortOrder: 2,
...
The nakshatra lord is the basis of the Vimshottari Dasha system — the Janma Nakshatra's lord becomes the first Mahadasha planet.`,
    ...
    relatedTerms: ["sign_lord", "nakshatra", "kp_star_lord", "vimshottari_dasha"],
    tags: ["attribute", "ruler", "nakshatra"],
  },
  {
    termKey: "house_placement", domain: "vedic", category: "planetary_attribute",
    title: "House (Bhava) Placement (भाव स्थिति)", sanskrit: "भाव", sortOrder: 3,
...
A planet's house placement is the most immediately useful piece of information — it tells you WHERE in life the planet operates.`,
    ...
    relatedTerms: ["sign_lord", "degree"],
    tags: ["attribute", "house", "bhava", "placement"],
  },
  {
    termKey: "degree", domain: "vedic", category: "planetary_attribute",
    title: "Degree (अंश)", sanskrit: "अंश", sortOrder: 4,
...
- Pushkara degrees: Exceptionally auspicious positions within signs`,
    ...
    relatedTerms: ["longitude", "pada", "nakshatra"],
    tags: ["attribute", "degree", "position", "precise"],
  },
  {
    termKey: "longitude", domain: "vedic", category: "planetary_attribute",
    title: "Longitude (स्फुट)", sanskrit: "स्फुट", sortOrder: 5,
...
The difference between Sayana and Nirayana longitude is the Ayanamsa (currently approximately 24°). This is why Vedic and Western charts show planets in different signs.`,
    ...
    relatedTerms: ["degree", "ayanamsa", "lahiri_ayanamsa"],
    tags: ["attribute", "astronomy", "position"],
  },
  {
    termKey: "retrograde", domain: "vedic", category: "planetary_attribute",
    title: "Retrograde — Vakri (वक्री)", sanskrit: "वक्री", sortOrder: 6,
...
- Past-life connections: Retrograde planets may indicate unfinished karmic business`,
    ...
    relatedTerms: ["cheshta_bala", "degree"],
    tags: ["attribute", "retrograde", "motion"],
  },
  {
    termKey: "combustion", domain: "vedic", category: "planetary_attribute",
    title: "Combustion — Asta (अस्त)", sanskrit: "अस्त", sortOrder: 7,
...
**Important nuance:** Mercury is frequently combust (never more than 28° from Sun) — strict combustion rules would make Mercury combust for a large percentage of people. Many astrologers use a narrower 5° threshold for Mercury or ignore its combustion entirely.`,
    ...
    relatedTerms: ["sun", "degree", "budha_aditya_yoga"],
    tags: ["attribute", "combustion", "weakness", "sun"],
  },
  {
    termKey: "dignity", domain: "vedic", category: "planetary_attribute",
    title: "Dignity (बल स्थिति)", sanskrit: "बल स्थिति", sortOrder: 8,
...
- Saturn: Aquarius 0°-20°`,
    ...
    relatedTerms: ["ucha", "neecha", "swakshetra", "vimsopaka"],
    tags: ["attribute", "dignity", "strength", "comprehensive"],
  },
];

// SECTION 15: KP SYSTEM (18 terms) — Task S2.6
const KP_ENTRIES: SeedEntry[] = [
  {
    termKey: "kp_system",
    domain: "kp",
    category: "kp_core",
    ...
    examples: [
      { title: "Marriage Prediction", content: "For marriage, check the Sub Lord of the 7th cusp. If the SL signifies houses 2, 7, and 11 (marriage-favorable houses), marriage is promised. The timing comes from the dasha of planets signifying these same houses." },
      { title: "KP Number (Horary)", content: "A querent asks 'Will I get the job?' and picks number 178. This maps to a specific degree in the zodiac, establishing the horary ascendant. The Sub Lord of the 10th cusp in this chart determines the answer." },
    ],
    relatedTerms: ["sub_lord", "star_lord", "sign_lord", "cuspal_analysis", "kp_horary", "significator"],
    tags: ["kp", "krishnamurti", "paddhati", "system", "sub-lord", "prediction"],
    sortOrder: 1,
  },
  {
    termKey: "sub_lord",
    ...
    examples: [
      { title: "SL Confirming Promise", content: "7th cusp SL is Venus, placed in the star of Mercury (lord of 2 and 11). Venus through its star lord connects to marriage-favorable houses 2, 7, 11 — marriage is strongly promised." },
      { title: "SL Denying Promise", content: "10th cusp SL is Saturn, in the star of Mars (lord of 6 and 12). Saturn through its star lord connects to separation houses — despite other positive factors, career stability is denied." },
    ],
    relatedTerms: ["kp_system", "star_lord", "sign_lord", "sub_sub_lord", "cuspal_analysis"],
    tags: ["kp", "sub-lord", "sl", "prediction", "decisive", "nakshatra-division"],
    sortOrder: 2,
  },
  {
    termKey: "star_lord",
    ...
    examples: [
      { title: "Planet in Star", content: "Venus is in Mrigashira nakshatra (Star Lord = Mars, owner of houses 4 and 9). Venus will primarily deliver 4th house (property, vehicles) and 9th house (fortune, travel) results, regardless of Venus's own house positions." },
    ],
    relatedTerms: ["sub_lord", "sign_lord", "kp_system", "nakshatra", "significator"],
    tags: ["kp", "star-lord", "nakshatra-lord", "nl", "signification"],
    sortOrder: 3,
  },
  {
    termKey: "sign_lord",
    ...
    examples: [
      { title: "KP Chain Example", content: "2nd cusp at 15° Taurus: RL = Venus (Taurus lord), NL = Moon (Rohini nakshatra), SL = Mars (Rohini sub). The broad domain is Venus (wealth, luxury), channeled through Moon (public, emotions), confirmed/denied by Mars's significations." },
    ],
    relatedTerms: ["sub_lord", "star_lord", "kp_system", "rashi_lord"],
    tags: ["kp", "sign-lord", "rashi-lord", "rl", "ownership"],
    sortOrder: 4,
  },
  {
    ...
    relatedTerms: ["sub_lord", "star_lord", "sign_lord", "kp_system"],
    tags: ["kp", "sub-sub-lord", "ss", "advanced", "precision", "timing"],
    sortOrder: 5,
  },
  {
    ...
    examples: [
      { title: "Career Query", content: "For career success, check the SL of the 10th cusp. If the SL is Jupiter, and Jupiter occupies the star of Mercury (lord of 2, 11), then Jupiter connects to money houses through the career cusp — professional success with financial rewards is indicated." },
      { title: "Health Query", content: "For health, check the SL of the 1st cusp (body) and 6th cusp (disease). If the 6th cusp SL signifies 1, 5, 11 — disease is cured. If it signifies 6, 8, 12 — chronic health issues persist." },
    ],
    relatedTerms: ["sub_lord", "kp_system", "significator", "cuspal_interlink"],
    tags: ["kp", "cuspal", "analysis", "house", "cusp", "prediction"],
    sortOrder: 6,
  },
  {
    ...
    relatedTerms: ["cuspal_analysis", "sub_lord", "significator", "kp_system"],
    tags: ["kp", "cuspal", "interlink", "promise", "house-connection"],
    sortOrder: 7,
  },
  {
    ...
    examples: [
      { title: "Significator Hierarchy", content: "For the 7th house (marriage): Level 1 — Venus is in the star of Mars who occupies the 7th. Level 2 — Mars occupies the 7th. Level 3 — Moon is in the star of Jupiter who owns the 7th. Level 4 — Jupiter owns the 7th. Venus (Level 1) is the strongest 7th-house significator." },
    ],
    relatedTerms: ["cuspal_analysis", "star_lord", "kp_system", "cuspal_interlink"],
    tags: ["kp", "significator", "house", "signification", "prediction", "hierarchy"],
    sortOrder: 8,
  },
  {
    ...
    examples: [
      { title: "Filtering Significators", content: "For marriage (houses 2, 7, 11), suppose Venus, Jupiter, Mercury, and Moon all signify these houses. The current Ruling Planets are Venus, Saturn, Mercury, and Rahu. Filtered result: Venus and Mercury (both significators AND ruling planets) are the strongest indicators — their dasha periods will trigger marriage." },
    ],
    relatedTerms: ["kp_system", "significator", "cuspal_analysis", "kp_horary"],
    tags: ["kp", "ruling-planets", "rp", "timing", "filter", "current-moment"],
    sortOrder: 9,
  },
  {
    termKey: "kp_horary",
    ...
    examples: [
      { title: "Horary for Travel", content: "Question: 'Will my visa be approved?' Number chosen: 156. The chart is cast with 156's corresponding ascendant. Check 3rd cusp SL (short travel), 9th cusp SL (foreign travel), and 12th cusp SL (foreign settlement). If these SLs connect to favorable houses, visa approval is indicated." },
    ],
    relatedTerms: ["kp_system", "sub_lord", "ruling_planets", "significator"],
    tags: ["kp", "horary", "prashna", "question", "249", "number"],
    sortOrder: 10,
  },
  {
    termKey: "kp_bhava",
    ...
    relatedTerms: ["cuspal_analysis", "kp_system", "sub_lord", "significator"],
    tags: ["kp", "bhava", "house", "placidus", "cusp", "division"],
    sortOrder: 11,
  },
  {
    termKey: "kp_fortuna",
    ...
    relatedTerms: ["kp_system", "sub_lord", "cuspal_analysis"],
    tags: ["kp", "fortuna", "fortune", "wealth", "arabic-parts", "sensitive-point"],
    sortOrder: 12,
  },
  {
    ...
    relatedTerms: ["star_lord", "nakshatra", "kp_system", "significator"],
    tags: ["kp", "nakshatra", "nadi", "connection", "star-lord", "groups"],
    sortOrder: 13,
  },
  {
    ...
    examples: [
      { title: "Dasha Convergence", content: "Currently running: Jupiter Maha / Venus Antar / Mercury Pratyantar. Jupiter signifies 4, 7. Venus signifies 2, 7, 11. Mercury signifies 7, 10. All three signify the 7th house — this is a strong marriage period. The exact month comes from the Sookshma dasha lord." },
    ],
    relatedTerms: ["vimshottari_dasha", "significator", "kp_system", "mahadasha"],
    tags: ["kp", "dasha", "timing", "vimshottari", "signification", "period"],
    sortOrder: 14,
  },
  {
    termKey: "kp_placidus",
    ...
    relatedTerms: ["kp_bhava", "cuspal_analysis", "kp_system"],
    tags: ["kp", "placidus", "house-system", "unequal", "cusp", "division"],
    sortOrder: 15,
  },
  {
    termKey: "kp_ayanamsa",
    ...
    relatedTerms: ["kp_system", "kp_placidus", "sub_lord"],
    tags: ["kp", "ayanamsa", "precession", "sidereal", "correction"],
    sortOrder: 16,
  },
  {
    ...
    relatedTerms: ["kp_system", "vimsopaka", "d1_rashi", "d9_navamsha"],
    tags: ["kp", "shodasha", "varga", "divisional", "strength", "dignity"],
    sortOrder: 17,
  },
  {
    ...
    relatedTerms: ["kp_horary", "sub_lord", "kp_system", "nakshatra"],
    tags: ["kp", "249", "mapping", "horary", "sub-lord", "division", "zodiac"],
    sortOrder: 18,
  },
];

// SECTION 16: MATCHMAKING — ASHTA KOOTA (8 terms) — Task S2.6
const KOOTA_ENTRIES: SeedEntry[] = [
  {
    termKey: "koota_varna",
    domain: "matchmaking",
    ...
    examples: [
      { title: "Full Score", content: "Groom: Moon in Aries (Kshatriya). Bride: Moon in Taurus (Vaishya). Kshatriya ≥ Vaishya → 1 point awarded." },
      { title: "Zero Score", content: "Groom: Moon in Gemini (Shudra). Bride: Moon in Cancer (Brahmin). Shudra < Brahmin → 0 points." },
    ],
    relatedTerms: ["koota_vashya", "koota_yoni", "koota_gana", "koota_nadi"],
    tags: ["matchmaking", "koota", "varna", "compatibility", "spiritual", "ego"],
    sortOrder: 1,
  },
  {
    ...
    examples: [
      { title: "Full Score", content: "Both partners have Moon in Manava (human) signs (Gemini + Libra). Same Vashya category → 2 points (mutual, balanced attraction)." },
      { title: "Partial Score", content: "Groom: Leo (Vanachara). Bride: Taurus (Chatushpada). Vanachara has partial dominance over Chatushpada → 1 point." },
    ],
    relatedTerms: ["koota_varna", "koota_yoni", "koota_graha_maitri", "koota_gana"],
    tags: ["matchmaking", "koota", "vashya", "attraction", "dominance", "compatibility"],
    sortOrder: 2,
  },
  {
    termKey: "koota_tara",
    ...
    examples: [
      { title: "Calculation", content: "Groom: Rohini (4th nakshatra). Bride: Hasta (13th nakshatra). Distance = 13 - 4 = 9. 9 ÷ 9 = remainder 0 → Ati Mitra (great friend) = favorable. Reverse: 4 + 27 - 13 = 18. 18 ÷ 9 = remainder 0 → also favorable. Both favorable → 3 points." },
    ],
    relatedTerms: ["koota_varna", "koota_yoni", "nakshatra", "koota_nadi"],
    tags: ["matchmaking", "koota", "tara", "dina", "fortune", "destiny", "nakshatra"],
    sortOrder: 3,
  },
  {
    termKey: "koota_yoni",
    ...
    examples: [
      { title: "Same Yoni", content: "Both partners have Moon in Horse (Ashwa) nakshatras: Ashwini and Shatabhisha. Same Yoni → 4 points (perfect physical compatibility)." },
      { title: "Enemy Yoni", content: "Groom: Ardra (Dog/Shvana). Bride: Magha (Rat/Mushaka). Dog and Rat are enemy Yonis → 1 point (significant physical mismatch)." },
    ],
    relatedTerms: ["koota_vashya", "koota_graha_maitri", "koota_nadi", "nakshatra"],
    tags: ["matchmaking", "koota", "yoni", "physical", "compatibility", "animal", "intimate"],
    sortOrder: 4,
  },
  {
    ...
    examples: [
      { title: "Full Score", content: "Groom: Moon in Cancer (lord = Moon). Bride: Moon in Scorpio (lord = Mars). Moon and Mars are mutual friends → 5 points (excellent mental wavelength)." },
      { title: "Zero Score", content: "Groom: Moon in Capricorn (lord = Saturn). Bride: Moon in Leo (lord = Sun). Saturn and Sun are mutual enemies → 0 points (fundamental thinking mismatch)." },
    ],
    relatedTerms: ["koota_varna", "koota_gana", "koota_bhakoot", "koota_nadi"],
    tags: ["matchmaking", "koota", "graha-maitri", "mental", "friendship", "compatibility", "intellect"],
    sortOrder: 5,
  },
  {
    termKey: "koota_gana",
    ...
    examples: [
      { title: "Same Gana", content: "Both Moons in Rakshasa nakshatras: Magha and Jyeshtha. Same Gana → 6 points. Despite both being 'Rakshasa,' they understand each other's intensity." },
      { title: "Worst Mismatch", content: "Groom: Pushya (Deva). Bride: Ashlesha (Rakshasa). Deva-Rakshasa → 0 points. The gentle Pushya native may find Ashlesha's manipulative tendencies deeply troubling." },
    ],
    relatedTerms: ["koota_varna", "koota_yoni", "koota_bhakoot", "koota_nadi", "gana"],
    tags: ["matchmaking", "koota", "gana", "temperament", "deva", "manushya", "rakshasa"],
    sortOrder: 6,
  },
  {
    ...
    examples: [
      { title: "6-8 Bhakoot Dosha", content: "Groom: Moon in Aries (1st sign). Bride: Moon in Virgo (6th sign). Distance = 6. Reverse = 8. This is a 6-8 pair → 0 points (Bhakoot Dosha: health/separation risk). BUT if Mars (Aries lord) and Mercury (Virgo lord) are friendly, dosha is cancelled → 7 points restored." },
      { title: "No Dosha", content: "Groom: Moon in Taurus (2nd). Bride: Moon in Libra (7th). Distance = 6 from Taurus, but Taurus-Libra are NOT in 2-12, 6-8, or 5-9 pair → 7 points (no dosha)." },
    ],
    relatedTerms: ["koota_nadi", "koota_graha_maitri", "koota_gana", "bhakoot_dosha"],
    tags: ["matchmaking", "koota", "bhakoot", "love", "finance", "prosperity", "family"],
    sortOrder: 7,
  },
  {
    termKey: "koota_nadi",
    ...
    examples: [
      { title: "Nadi Dosha", content: "Groom: Ashwini (Aadi Nadi). Bride: Punarvasu (Aadi Nadi). Same Nadi → 0 points (Nadi Dosha). This indicates potential health issues for offspring." },
      { title: "Dosha Cancelled", content: "Both have Aadi Nadi, but Groom is in Ashwini (Aries) and Bride is in Ardra (Gemini). Same Nadi but different Moon signs → Nadi Dosha cancelled → 8 points restored." },
    ],
    relatedTerms: ["koota_bhakoot", "koota_gana", "koota_graha_maitri", "nadi_dosha", "nadi"],
    tags: ["matchmaking", "koota", "nadi", "health", "genetic", "progeny", "critical"],
    sortOrder: 8,
  },
];

// SECTION 17: MATCHMAKING — DOSHAS (3 terms) — Task S2.6
const MATCH_DOSHA_ENTRIES: SeedEntry[] = [
  {
    ...
    examples: [
      { title: "Both Manglik (Cancelled)", content: "Groom: Mars in 7th house. Bride: Mars in 8th house. Both are Manglik → doshas mutually cancel. No marriage impediment." },
      { title: "One Manglik (Active)", content: "Groom: Mars in 1st house (Manglik). Bride: No Mars in dosha houses (non-Manglik). Mismatch → check for cancellation: Is Mars in own sign? Is Jupiter aspecting? If no cancellations, dosha is active." },
    ],
    relatedTerms: ["manglik_dosha", "koota_bhakoot", "koota_nadi", "mars"],
    tags: ["matchmaking", "dosha", "manglik", "mars", "marriage", "compatibility", "cancellation"],
    sortOrder: 1,
  },
  {
    ...
    examples: [
      { title: "Active Nadi Dosha", content: "Groom: Ashwini (Aadi Nadi, Aries). Bride: Shatabhisha (Aadi Nadi, Aquarius). Same Nadi, different signs, different nakshatras → check: different Moon signs (Aries vs Aquarius) → Dosha CANCELLED." },
      { title: "Truly Active", content: "Groom: Krittika pada 2 (Aadi Nadi, Taurus). Bride: Krittika pada 3 (Aadi Nadi, Taurus). Same Nadi, same nakshatra, same Moon sign, but different padas → Dosha CANCELLED due to different padas." },
    ],
    relatedTerms: ["koota_nadi", "nadi", "manglik_dosha_match", "bhakoot_dosha"],
    tags: ["matchmaking", "dosha", "nadi", "health", "progeny", "cancellation"],
    sortOrder: 2,
  },
  {
    ...
    examples: [
      { title: "6-8 Bhakoot (Cancelled)", content: "Groom: Moon in Taurus (Venus). Bride: Moon in Sagittarius (Jupiter). Distance = 8 from Taurus → 6-8 pair → 0 points. BUT Venus and Jupiter are mutual friends → Bhakoot Dosha cancelled → 7 points restored." },
      { title: "2-12 Bhakoot (Active)", content: "Groom: Moon in Leo (Sun). Bride: Moon in Cancer (Moon). Distance = 12 from Leo → 2-12 pair → 0 points. Sun and Moon are friends → cancelled → 7 points restored. (This particular 2-12 is almost always cancelled because Sun-Moon are natural friends.)" },
    ],
    relatedTerms: ["koota_bhakoot", "nadi_dosha_match", "manglik_dosha_match"],
    tags: ["matchmaking", "dosha", "bhakoot", "finance", "separation", "6-8", "cancellation"],
    sortOrder: 3,
  },
];

// SECTION 18: MATCHMAKING — SCORE THRESHOLDS (4 terms) — Task S2.7
const MATCH_SCORE_ENTRIES: SeedEntry[] = [
  {
    ...
    examples: [
      { title: "Score Breakdown", content: "Varna: 1, Vashya: 2, Tara: 3, Yoni: 3, Graha Maitri: 5, Gana: 6, Bhakoot: 7, Nadi: 8 = 35/36 — Outstanding match with only minor Yoni mismatch." },
    ],
    relatedTerms: ["match_score_good", "match_score_average", "koota_nadi", "koota_bhakoot"],
    tags: ["matchmaking", "score", "excellent", "threshold", "compatibility", "28-36"],
    sortOrder: 1,
  },
  {
    ...
    relatedTerms: ["match_score_excellent", "match_score_average", "koota_gana", "koota_graha_maitri"],
    tags: ["matchmaking", "score", "good", "threshold", "compatibility", "21-27"],
    sortOrder: 2,
  },
  {
    ...
    relatedTerms: ["match_score_good", "match_score_below_average", "koota_nadi", "manglik_dosha_match"],
    tags: ["matchmaking", "score", "average", "threshold", "minimum", "18-20"],
    sortOrder: 3,
  },
  {
    ...
    relatedTerms: ["match_score_average", "match_score_good", "manglik_dosha_match", "nadi_dosha_match"],
    tags: ["matchmaking", "score", "below-average", "incompatible", "remedies", "0-17"],
    sortOrder: 4,
  },
];

// SECTION 19: MUHURTA (10 terms) — Task S2.6
const MUHURTA_ENTRIES: SeedEntry[] = [
  {
    termKey: "muhurta",
    domain: "vedic",
    category: "muhurta",
    ...
    examples: [
      { title: "Wedding Muhurta", content: "For a wedding, favorable factors: Tithi = Dwitiya (2nd lunar day), Nakshatra = Rohini (ruled by Moon, excellent for marriage), Yoga = Siddha (success), Day = Thursday (Jupiter's day). Avoid: Rahu Kaal window, Amavasya (new moon)." },
    ],
    relatedTerms: ["abhijit_muhurta", "brahma_muhurta", "rahu_kaal", "tithi", "nakshatra"],
    tags: ["muhurta", "timing", "auspicious", "electional", "vedic", "ceremony"],
    sortOrder: 1,
  },
  {
    ...
    examples: [
      { title: "Practical Use", content: "Need to sign an important contract today, but the Panchanga shows unfavorable Tithi and Nakshatra. Solution: Sign during Abhijit Muhurta (today: 12:02 PM - 12:48 PM). The Abhijit window neutralizes the negative factors." },
    ],
    relatedTerms: ["muhurta", "brahma_muhurta", "rahu_kaal", "panchanga"],
    tags: ["muhurta", "abhijit", "auspicious", "noon", "victorious", "universal"],
    sortOrder: 2,
  },
  {
    ...
    relatedTerms: ["muhurta", "abhijit_muhurta", "panchanga"],
    tags: ["muhurta", "brahma", "spiritual", "meditation", "pre-dawn", "sadhana"],
    sortOrder: 3,
  },
  {
    termKey: "rahu_kaal",
    ...
    examples: [
      { title: "Monday Rahu Kaal", content: "Sunrise: 6:30 AM. Sunset: 6:30 PM. Daytime = 12 hours. Each segment = 1.5 hours. Monday = 2nd segment → Rahu Kaal = 7:30 AM - 9:00 AM. Avoid scheduling important meetings in this window." },
      { title: "Seasonal Shift", content: "In summer (longer days), each segment is longer, so Rahu Kaal expands. In winter (shorter days), it contracts. Always calculate from actual sunrise/sunset, not fixed times." },
    ],
    relatedTerms: ["gulika_kaal", "yamagandam", "muhurta", "rahu"],
    tags: ["muhurta", "rahu-kaal", "inauspicious", "timing", "avoid", "daily"],
    sortOrder: 4,
  },
  {
    termKey: "gulika_kaal",
    ...
    relatedTerms: ["rahu_kaal", "yamagandam", "muhurta", "saturn"],
    tags: ["muhurta", "gulika", "mandi", "inauspicious", "saturn", "timing"],
    sortOrder: 5,
  },
  {
    termKey: "yamagandam",
    ...
    relatedTerms: ["rahu_kaal", "gulika_kaal", "muhurta"],
    tags: ["muhurta", "yamagandam", "yama", "inauspicious", "danger", "travel"],
    sortOrder: 6,
  },
  {
    ...
    relatedTerms: ["tithi", "muhurta", "panchanga", "muhurta_nakshatra"],
    tags: ["muhurta", "tithi", "auspicious", "lunar-day", "timing", "panchanga"],
    sortOrder: 7,
  },
  {
    ...
    relatedTerms: ["nakshatra", "muhurta", "muhurta_tithi", "pushya"],
    tags: ["muhurta", "nakshatra", "auspicious", "lunar-mansion", "timing", "pushya"],
    sortOrder: 8,
  },
  {
    ...
    relatedTerms: ["yoga_panchanga", "vara", "muhurta", "muhurta_tithi", "muhurta_nakshatra"],
    tags: ["muhurta", "yoga", "weekday", "vara", "auspicious", "siddha", "timing"],
    sortOrder: 9,
  },
  {
    ...
    examples: [
      { title: "Category Matters", content: "Tuesday with Ashwini nakshatra: EXCELLENT for surgery (Mars = surgeon, Ashwini = medical healing). TERRIBLE for a wedding (Mars = conflict in marriage). Same day, opposite recommendations based on category." },
    ],
    relatedTerms: ["muhurta", "muhurta_tithi", "muhurta_nakshatra", "muhurta_yoga_day"],
    tags: ["muhurta", "category", "wedding", "property", "travel", "vehicle", "business"],
    sortOrder: 10,
  },
];

// SECTION 20: NUMEROLOGY (20 terms) — Task S2.7
const NUMEROLOGY_ENTRIES: SeedEntry[] = [
  {
    ...
    examples: [
      { title: "Name Calculation", content: "Name: RAMA → R(2) + A(1) + M(4) + A(1) = 8. The Chaldean value of RAMA is 8, associated with Saturn — authority, discipline, and material success." },
      { title: "Compound Number", content: "Name: KRISHNA → K(2)+R(2)+I(1)+S(3)+H(5)+N(5)+A(1) = 19. Compound 19 ('The Prince of Heaven') is one of the most fortunate numbers, reducing to 1+9=10→1+0=1 (Sun — leadership)." },
    ],
    relatedTerms: ["destiny_number", "birth_number", "name_number", "compound_number"],
    tags: ["numerology", "chaldean", "system", "babylonian", "vibration", "ancient"],
    sortOrder: 1,
  },
  {
    ...
    examples: [
      { title: "Destiny 8", content: "Birth date: 26/01/1994 → 2+6+0+1+1+9+9+4 = 32 → 3+2 = 5. Destiny 5: This person is meant to experience freedom, travel, versatility, and change. Careers in media, travel, sales, or any field requiring adaptability will fulfill their destiny." },
    ],
    relatedTerms: ["birth_number", "name_number", "chaldean_system", "compound_number"],
    tags: ["numerology", "destiny", "life-path", "birth-date", "purpose", "core"],
    sortOrder: 2,
  },
  {
    ...
    relatedTerms: ["destiny_number", "name_number", "chaldean_system", "personal_year"],
    tags: ["numerology", "birth-number", "personality", "day", "core", "character"],
    sortOrder: 3,
  },
  {
    termKey: "name_number",
    ...
    examples: [
      { title: "Name Change Impact", content: "Original: SANJAY = S(3)+A(1)+N(5)+J(1)+A(1)+Y(1) = 12 → 3. Changed to: SANJEY = S(3)+A(1)+N(5)+J(1)+E(5)+Y(1) = 16 → 7. One letter change shifts the Name Number from 3 (creative, social) to 7 (analytical, spiritual) — dramatically different energy." },
    ],
    relatedTerms: ["destiny_number", "birth_number", "chaldean_system", "compound_number"],
    tags: ["numerology", "name-number", "expression", "vibration", "name-change", "core"],
    sortOrder: 4,
  },
  {
    ...
    relatedTerms: ["chaldean_system", "destiny_number", "name_number", "karmic_debt_number"],
    tags: ["numerology", "compound", "two-digit", "karmic", "chaldean", "meaning"],
    sortOrder: 5,
  },
  {
    ...
    examples: [
      { title: "Karmic 16", content: "Name reduces to 16/7: The person may experience cycles of building something impressive (career, relationship) only to see it collapse due to ego or pride. Each collapse is an opportunity for deeper spiritual understanding. Once humility is learned, the 7's natural wisdom flourishes." },
    ],
    relatedTerms: ["compound_number", "destiny_number", "chaldean_system"],
    tags: ["numerology", "karmic-debt", "13", "14", "16", "19", "past-life", "spiritual"],
    sortOrder: 6,
  },
  {
    ...
    examples: [
      { title: "Year 5 Surprise", content: "In a Personal Year 5, expect the unexpected. A person planning stability (buying a house, settling down) may find circumstances pushing them toward change — a job offer abroad, a surprise relocation, or meeting someone who transforms their worldview." },
    ],
    relatedTerms: ["destiny_number", "personal_month", "personal_day", "chaldean_system"],
    tags: ["numerology", "personal-year", "timing", "cycle", "9-year", "prediction"],
    sortOrder: 7,
  },
  {
    ...
    relatedTerms: ["personal_year", "personal_day", "destiny_number"],
    tags: ["numerology", "personal-month", "timing", "cycle", "monthly", "sub-cycle"],
    sortOrder: 8,
  },
  {
    ...
    relatedTerms: ["personal_month", "personal_year", "daily_forecast"],
    tags: ["numerology", "personal-day", "timing", "daily", "cycle", "planning"],
    sortOrder: 9,
  },
  {
    ...
    relatedTerms: ["destiny_number", "name_number", "birth_number", "pinnacle_number"],
    tags: ["numerology", "maturity", "aging", "potential", "second-half", "evolution"],
    sortOrder: 10,
  },
  {
    ...
    relatedTerms: ["destiny_number", "maturity_number", "personal_year", "challenge_number"],
    tags: ["numerology", "pinnacle", "life-phase", "timing", "decades", "transition"],
    sortOrder: 11,
  },
  {
    termKey: "lo_shu_grid",
    ...
    examples: [
      { title: "Grid Analysis", content: "Born 15/11/1985: Digits = 1,5,1,1,1,9,8,5. Grid: Cell 1 has FOUR 1s (excessive individuality/ego), Cell 5 has TWO 5s (strong freedom need), Cell 9 has ONE 9, Cell 8 has ONE 8. Missing: 2,3,4,6,7 — indicates gaps in cooperation (2), creativity (3), stability (4), responsibility (6), and spirituality (7)." },
    ],
    relatedTerms: ["chaldean_system", "birth_number", "destiny_number"],
    tags: ["numerology", "lo-shu", "grid", "magic-square", "chinese", "birth-date", "visual"],
    sortOrder: 12,
  },
  {
    ...
    relatedTerms: ["destiny_number", "birth_number", "name_number", "chaldean_system"],
    tags: ["numerology", "lucky", "numbers", "practical", "selection", "daily"],
    sortOrder: 13,
  },
  {
    ...
    relatedTerms: ["lucky_numbers", "chaldean_system", "birth_number"],
    tags: ["numerology", "mobile", "phone", "number", "vibration", "practical"],
    sortOrder: 14,
  },
  {
    ...
    relatedTerms: ["name_number", "chaldean_system", "compound_number", "lucky_numbers"],
    tags: ["numerology", "business", "name", "brand", "commercial", "success"],
    sortOrder: 15,
  },
  {
    ...
    relatedTerms: ["destiny_number", "birth_number", "name_number", "chaldean_system"],
    tags: ["numerology", "compatibility", "relationship", "love", "partnership", "matching"],
    sortOrder: 16,
  },
  {
    ...
    relatedTerms: ["name_number", "chaldean_system", "destiny_number", "nakshatra"],
    tags: ["numerology", "baby-name", "naming", "child", "newborn", "optimization"],
    sortOrder: 17,
  },
  {
    ...
    relatedTerms: ["personal_day", "personal_month", "personal_year", "lucky_numbers"],
    tags: ["numerology", "daily", "forecast", "prediction", "color", "timing"],
    sortOrder: 18,
  },
  {
    ...
    relatedTerms: ["destiny_number", "birth_number", "name_number", "pinnacle_number", "lo_shu_grid", "personal_year"],
    tags: ["numerology", "blueprint", "comprehensive", "profile", "complete", "premium"],
    sortOrder: 19,
  },
  {
    ...
    relatedTerms: ["chaldean_system", "destiny_number", "birth_number", "compound_number"],
    tags: ["numerology", "meanings", "1-9", "archetypes", "planets", "foundation"],
    sortOrder: 20,
  },
];

// SECTION 21: GENERAL (10 terms) — Task S2.7
const GENERAL_ENTRIES: SeedEntry[] = [
  {
    termKey: "kundali",
    domain: "vedic",
    category: "general",
    ...
    examples: [
      { title: "Chart Reading Start", content: "Ascendant in Leo (ruled by Sun) → The person leads with confidence, charisma, and authority. Sun in the 10th house → Career is central to identity. Jupiter aspects the Ascendant → Wisdom, optimism, and good fortune color the personality." },
    ],
    relatedTerms: ["navagraha", "rashi", "nakshatra", "bhava", "lagna"],
    tags: ["general", "kundali", "birth-chart", "horoscope", "foundation", "vedic"],
    sortOrder: 1,
  },
  {
    termKey: "lagna",
    ...
    relatedTerms: ["kundali", "rashi", "bhava", "sign_lord"],
    tags: ["general", "lagna", "ascendant", "rising-sign", "1st-house", "foundation"],
    sortOrder: 2,
  },
  {
    termKey: "ayanamsa",
    ...
    relatedTerms: ["kp_ayanamsa", "lagna", "kundali", "rashi"],
    tags: ["general", "ayanamsa", "precession", "sidereal", "tropical", "correction", "technical"],
    sortOrder: 3,
  },
  {
    termKey: "transit",
    ...
    relatedTerms: ["sade_sati", "ashtakavarga", "vimshottari_dasha", "kundali"],
    tags: ["general", "transit", "gochara", "current", "timing", "real-time", "movement"],
    sortOrder: 4,
  },
  {
    termKey: "retrograde",
    ...
    relatedTerms: ["navagraha", "transit", "kundali", "mercury"],
    tags: ["general", "retrograde", "vakri", "backward", "strength", "review"],
    sortOrder: 5,
  },
  {
    termKey: "combustion",
    ...
    relatedTerms: ["sun", "navagraha", "dignity", "kundali"],
    tags: ["general", "combustion", "asta", "sun", "proximity", "weakness"],
    sortOrder: 6,
  },
  {
    ...
    examples: [
      { title: "Jupiter's Triple Aspect", content: "Jupiter in the 1st house aspects: 5th house (children, creativity — benefic), 7th house (marriage, partnerships — benefic), 9th house (fortune, spirituality — benefic). Three houses simultaneously blessed by Jupiter's wisdom." },
    ],
    relatedTerms: ["navagraha", "kundali", "bhava", "jupiter", "saturn", "mars"],
    tags: ["general", "aspect", "drishti", "glance", "influence", "special-aspect"],
    sortOrder: 7,
  },
  {
    ...
    relatedTerms: ["kundali", "lagna", "bhava", "significator"],
    tags: ["general", "house", "bhava", "signification", "meaning", "12-houses", "life-areas"],
    sortOrder: 8,
  },
  {
    ...
    examples: [
      { title: "Saturn as Yogakaraka", content: "For Taurus and Libra Lagnas, Saturn owns both a Kendra (angular house) and a Trikona (trine house), making it the Yogakaraka — the single most beneficial planet in the chart, despite being a natural malefic. Saturn's dasha becomes the most productive period." },
    ],
    relatedTerms: ["navagraha", "lagna", "kundali", "jupiter", "saturn", "yogakaraka"],
    tags: ["general", "benefic", "malefic", "shubha", "papa", "classification", "functional"],
    sortOrder: 9,
  },
  {
    ...
    examples: [
      { title: "Saturn Remedy", content: "Weak Saturn causing career delays → Behavioral remedy: respect elders, be punctual, take responsibility. Charity: donate black items on Saturdays. Mantra: 'Om Sham Shanaishcharaya Namah' (108 times on Saturdays). Gemstone: Blue Sapphire ONLY if Saturn is a functional benefic for your Lagna." },
    ],
    relatedTerms: ["navagraha", "kundali", "benefic_malefic", "saturn", "jupiter"],
    tags: ["general", "remedies", "upaya", "gemstone", "mantra", "charity", "healing"],
    sortOrder: 10,
  },
];
...
async function main() {
  console.log("🌱 Knowledge Service — Seeding knowledge entries...\n");
  ...
  const allEntries: SeedEntry[] = [
    ...
    ...RASHI_ENTRIES,
    ...NAKSHATRA_ENTRIES,
    ...DASHA_SYSTEM_ENTRIES,
    ...DASHA_LEVEL_ENTRIES,
    ...VARGA_ENTRIES,
    ...YOGA_ENTRIES,
    ...DOSHA_ENTRIES,
    ...SHADBALA_ENTRIES,
    ...ASHTAKAVARGA_ENTRIES,
    ...DIGNITY_ENTRIES,
    ...ATTRIBUTE_ENTRIES,
    ...KP_ENTRIES,
    ...KOOTA_ENTRIES,
  ...
  // Clean re-seed if requested
  if (process.env.CLEAN_SEED === "1") {
    console.log("🧹 CLEAN_SEED=1 — Deleting all existing entries...");
    await prisma.knowledgeEntry.deleteMany();
    console.log("   Done.\n");
  }

  console.log(`📚 Seeding ${allEntries.length} entries...\n`);
  ...
  for (let i = 0; i < allEntries.length; i++) {
    ...
    // Progress indicator every 25 entries
    if ((i + 1) % 25 === 0 || i === allEntries.length - 1) {
      console.log(`   Progress: ${i + 1}/${allEntries.length}`);
    }
  }
  ...
  // Print stats
  const stats = await prisma.knowledgeEntry.groupBy({
    by: ["domain"],
    _count: { _all: true },
  });
  console.log("\n📊 Entries by domain:");
  for (const s of stats) {
    console.log(`   ${s.domain}: ${s._count._all}`);
  }
}

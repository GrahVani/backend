...
async function verify() {
  ...
  try {
    ...
    if (periods2.length > 0) {
      console.log(
        "First 3 Periods (40y from mahadashas):",
        JSON.stringify(periods2.slice(0, 3), null, 2),
      );
    }
    ...
    if (t2.data.tribhagi_dasha_info) {
      console.log(
        "Tribhagi Dasha Info:",
        JSON.stringify(t2.data.tribhagi_dasha_info, null, 2).substring(0, 500),
      );
    }
    ...
    if (
      periods1.length > 0 &&
      periods2.length > 0 &&
      periods1[0].start_date === periods2[0].start_date &&
      periods1[0].end_date === periods2[0].end_date
    ) {
